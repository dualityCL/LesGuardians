...
public class World {
    private static Map<Integer, Conta> Cuentas = new TreeMap<Integer, Conta>();
    private static Map<String, Integer> CuentasPorNombre = new TreeMap<String, Integer>();
    private static Map<Integer, Personagens> Personajes = new TreeMap<Integer, Personagens>();
    private static Map<Short, Maps> Mapas = new TreeMap<Short, Maps>();
    ...
    private static Map<Integer, Spell> Hechizos = new TreeMap<Integer, Spell>();
    private static Map<Integer, Objeto.ObjetoModelo> ObjModelos = new TreeMap<Integer, Objeto.ObjetoModelo>();
    private static Map<Integer, Set_Vivo> Objevivos = new TreeMap<Integer, Set_Vivo>();
    private static Map<Integer, MOB_tmpl> MobModelos = new TreeMap<Integer, MOB_tmpl>();
    private static Map<Integer, NPC_tmpl> NPCModelos = new TreeMap<Integer, NPC_tmpl>();
    private static Map<Integer, NPC_tmpl.PreguntaNPC> NPCPreguntas = new TreeMap<Integer, NPC_tmpl.PreguntaNPC>();
    private static Map<Integer, NPC_tmpl.RespuestaNPC> NPCRespuesta = new TreeMap<Integer, NPC_tmpl.RespuestaNPC>();
    private static Map<Integer, ObjInteractivoModelo> ObjInteractivos = new TreeMap<Integer, ObjInteractivoModelo>();
    private static Map<Integer, Dragossauros> Dragopavos = new TreeMap<Integer, Dragossauros>();
    ...
    private static Map<Integer, Profissao> Oficios = new TreeMap<Integer, Profissao>();
    ...
    private static Map<Integer, Guild> Gremios = new TreeMap<Integer, Guild>();
    private static Map<Integer, Casa> Casas = new TreeMap<Integer, Casa>();
    private static Map<Integer, Mercador> PuestosMercadillos = new TreeMap<Integer, Mercador>();
    private static Map<Integer, Map<Integer, ArrayList<Mercador.ObjetoMercadillo>>> ObjMercadillos = new HashMap<Integer, Map<Integer, ArrayList<Mercador.ObjetoMercadillo>>>();
    private static Map<Integer, Personagens> Esposos = new TreeMap<Integer, Personagens>();
    private static Map<Integer, Animacao> Animaciones = new TreeMap<Integer, Animacao>();
    private static Map<Integer, Desafios> Retos = new TreeMap<Integer, Desafios>();
    private static Map<Integer, Stockage> Tiendas = new TreeMap<Integer, Stockage>();
    private static Map<Integer, Pets> Mascotas = new TreeMap<Integer, Pets>();
    private static Map<Integer, Pets.MascotaModelo> MascotasModelos = new TreeMap<Integer, Pets.MascotaModelo>();
    private static Map<Short, Maps.Cercado> Cercados = new TreeMap<Short, Maps.Cercado>();
    ...
    private static Map<Integer, Coletor> Recaudadores = new TreeMap<Integer, Coletor>();
    private static Map<Integer, RankPVP> RankingsPVP = new TreeMap<Integer, RankPVP>();
    private static CopyOnWriteArrayList<Personagens> Koliseo1 = new CopyOnWriteArrayList();
    private static CopyOnWriteArrayList<Personagens> Koliseo2 = new CopyOnWriteArrayList();
    private static CopyOnWriteArrayList<Personagens> Koliseo3 = new CopyOnWriteArrayList();
    private static CopyOnWriteArrayList<Personagens.GrupoKoliseo> GrupoKoliseo1 = new CopyOnWriteArrayList();
    private static CopyOnWriteArrayList<Personagens.GrupoKoliseo> GrupoKoliseo2 = new CopyOnWriteArrayList();
    private static CopyOnWriteArrayList<Personagens.GrupoKoliseo> GrupoKoliseo3 = new CopyOnWriteArrayList();
    private static Map<Integer, Incarnacao> Encarnaciones = new TreeMap<Integer, Incarnacao>();
    ...
    public static Personagens _salvador;
    ...
    public static synchronized void salvarServidor() {
        ...
        try {
            ...
            try {
                ...
                Thread.sleep(2000L);
                TreeMap<Integer, Conta> _cuentas = new TreeMap<Integer, Conta>();
                _cuentas.putAll(Cuentas);
                TreeMap<Integer, Personagens> _personajes = new TreeMap<Integer, Personagens>();
                _personajes.putAll(Personajes);
                TreeMap<Integer, Set_Vivo> _objevivos = new TreeMap<Integer, Set_Vivo>();
                _objevivos.putAll(Objevivos);
                TreeMap<Integer, Incarnacao> _encarnaciones = new TreeMap<Integer, Incarnacao>();
                _encarnaciones.putAll(Encarnaciones);
                TreeMap<Integer, Stockage> _tiendas = new TreeMap<Integer, Stockage>();
                _tiendas.putAll(Tiendas);
                TreeMap<Integer, Prisma> _prismas = new TreeMap<Integer, Prisma>();
                _prismas.putAll(Prismas);
                TreeMap<Integer, Pets> _mascotas = new TreeMap<Integer, Pets>();
                _mascotas.putAll(Mascotas);
                TreeMap<Integer, Guild> _gremios = new TreeMap<Integer, Guild>();
                _gremios.putAll(Gremios);
                TreeMap<Integer, Coletor> _recaudadores = new TreeMap<Integer, Coletor>();
                _recaudadores.putAll(Recaudadores);
                TreeMap<Integer, Dragossauros> _dragopavos = new TreeMap<Integer, Dragossauros>();
                _dragopavos.putAll(Dragopavos);
                TreeMap<Integer, RankPVP> _ranking = new TreeMap<Integer, RankPVP>();
                _ranking.putAll(RankingsPVP);
                System.out.println("Salvando contas");
                for (Conta cuenta : _cuentas.values()) {
                    if (!cuenta.enLinea()) continue;
                    Thread.sleep(1000L);
                    SQLManager.SALVAR_CUENTA(cuenta);
                }
                SQLManager.ACTUALIZAR_NPC_KAMAS(World.getNPCModelo(408));
                System.out.println("Salvando personagens");
                for (Personagens perso : _personajes.values()) {
                    if (!perso.enLinea()) continue;
                    Thread.sleep(50L);
                    SQLManager.SALVAR_PERSONAJE(perso, true);
                }
                System.out.println("Salvando itens vivos.");
                for (Set_Vivo objevivo : _objevivos.values()) {
                    Thread.sleep(50L);
                    SQLManager.SALVAR_OBJEVIVO(objevivo);
                }
                System.out.println("Salvando incarnacoes.");
                for (Incarnacao encarnacion : _encarnaciones.values()) {
                    Thread.sleep(50L);
                    SQLManager.REPLACE_ENCARNACION(encarnacion);
                }
                System.out.println("Salvando itens de mercador.");
                for (Stockage tienda : _tiendas.values()) {
                    Thread.sleep(100L);
                ...
                for (Dragossauros montura : _dragopavos.values()) {
                    Thread.sleep(100L);
                ...
                for (RankPVP rank : _ranking.values()) {
                    Thread.sleep(10L);
                ...
                ArrayList<Mercador.ObjetoMercadillo> listaObjMerca = new ArrayList<Mercador.ObjetoMercadillo>();
                for (Mercador puesto : PuestosMercadillos.values()) {
                    Thread.sleep(10L);
    ...
    public static void addRespuestaNPC(NPC_tmpl.RespuestaNPC respuesta) {
        NPCRespuesta.put(respuesta.getID(), respuesta);
    }

    public static NPC_tmpl.RespuestaNPC getRespuestaNPC(int id) {
    ...
    public static Conta getCuenta(int id) {
        return Cuentas.get(id);
    }

    public static void addPreguntaNPC(NPC_tmpl.PreguntaNPC pregunta) {
        NPCPreguntas.put(pregunta.getID(), pregunta);
    }

    public static NPC_tmpl.PreguntaNPC getPreguntaNPC(int id) {
        return NPCPreguntas.get(id);
    }

    public static NPC_tmpl getNPCModelo(int id) {
        return NPCModelos.get(id);
    }

    public static void addNpcModelo(NPC_tmpl npcModelo) {
        NPCModelos.put(npcModelo.getID(), npcModelo);
    }

    public static Maps getMapa(short id) {
        return Mapas.get(id);
    }

    public static void addMapa(Maps mapa) {
        if (!Mapas.containsKey(mapa.getID())) {
            Mapas.put(mapa.getID(), mapa);
        }
    }

    public static Maps mapaPorCoordXYContinente(int mapaX, int mapaY, int idContinente) {
        for (Maps mapa : Mapas.values()) {
    ...
    public static Personagens getPjPorNombre(String nombre) {
        ArrayList<Personagens> Ps = new ArrayList<Personagens>();
        Ps.addAll(Personajes.values());
        for (Personagens perso : Ps) {
    ...
    public static synchronized void eliminarPj(Personagens perso, boolean totalmente) {
        World.desconectarPerso(perso);
        for (Casa casa : Casas.values()) {
        ...
        for (Maps.Cercado cercado : Cercados.values()) {
            ...
            for (int i = 0; i < n; ++i) {
                ...
                World.borrarDragopavo(id);
            }
            if (x.length() > 0) {
                SQLManager.DELETE_DRAGOPAVO_LISTA(x);
            }
            cercado = new Maps.Cercado(0, cercado.getMapa(), cercado.getCeldaID(), cercado.getTama\u00f1o(), -1, 3000000, cercado.getColocarCelda(), "", cercado.getPuerta(), "", cercado.getCantObjMax(), "");
            SQLManager.UPDATE_CERCADO(cercado);
        }
        if (perso.getMiembroGremio() != null) {
            SQLManager.DELETE_MIEMBRO_GREMIO(perso.getID());
        }
        World.delRankingPVP(perso.getID());
        if (totalmente) {
    ...
    public static long getExpMinPersonaje(int nivel) {
        ...
        return World.Experiencia.get((Object)Integer.valueOf((int)nivel))._personaje;
    }
    ...
    public static long getExpMaxPersonaje(int nivel) {
        ...
        return World.Experiencia.get((Object)Integer.valueOf((int)(nivel + 1)))._personaje;
    }
    ...
    public static long getExpMaxEncarnacion(int nivel) {
        ...
        return World.Experiencia.get((Object)Integer.valueOf((int)(nivel + 1)))._encarnacion;
    }
    ...
    public static Profissao getOficio(int id) {
        return Oficios.get(id);
    }

    public static void addOficio(Profissao oficio) {
    ...
    public static Conta getCuentaPorApodo(String apodo) {
        for (Conta C : Cuentas.values()) {
    ...
    public static Guild getGremio(int i) {
    ...
    public static long getXPMaxGremio(int nivel) {
        ...
        return World.Experiencia.get((Object)Integer.valueOf((int)(nivel + 1)))._gremio;
    }
    ...
    public static short getCeldaCercadoPorMapaID(short i) {
        Maps.Cercado cercado = World.getMapa(i).getCercado();
    ...
    public static void desconectarPerso(Personagens perso) {
    ...
    public static synchronized Objeto objetoIniciarServer(int id, int modelo, int cant, int pos, String strStats, int idObvi) {
        Objeto.ObjetoModelo objModelo = World.getObjModelo(modelo);
        if (objModelo == null) {
        ...
        if (objModelo.getTipo() == 85) {
            return new Almas(id, cant, modelo, pos, strStats);
        }
        return new Objeto(id, modelo, cant, pos, strStats, idObvi);
    }

    public static void addHechizo(Spell hechizo) {
    ...
    public static Spell getHechizo(int id) {
    ...
    public static void addMobModelo(int id, MOB_tmpl mob) {
        MobModelos.put(id, mob);
    }

    public static MOB_tmpl getMobModelo(int id) {
        return MobModelos.get(id);
    }

    public static ArrayList<Personagens> getPJsEnLinea() {
        ArrayList<Personagens> enLinea = new ArrayList<Personagens>();
        Map<Integer, Personagens> personajesO = Personajes;
        for (Personagens perso : personajesO.values()) {
    ...
    public static synchronized void addObjeto(Objeto obj, boolean salvarSQL) {
        ...
        if (obj.getID() == 0) {
            obj.setID(World.getSigIDObjeto());
        }
    ...
    public static Dragossauros getDragopavoPorID(int id) {
        return Dragopavos.get(id);
    }

    public static synchronized void addDragopavo(Dragossauros DP) {
    ...
    public static Mercador getPuestoMerca(int mapaID) {
        return PuestosMercadillos.get(mapaID);
    }

    public static synchronized void addObjMercadillo(int cuentaID, int idPuestoMerca, Mercador.ObjetoMercadillo objMercadillo) {
    ...
    public static synchronized void borrarObjMercadillo(int cuentaID, int idPuestoMerca, Mercador.ObjetoMercadillo objMerca) {
    ...
    public static int cantObjMercadillos() {
        int cantidad = 0;
        for (Map<Integer, ArrayList<Mercador.ObjetoMercadillo>> tempCuenta : ObjMercadillos.values()) {
            for (ArrayList<Mercador.ObjetoMercadillo> objMercadillo : tempCuenta.values()) {
    ...
    public static synchronized void addPuestoMercadillo(Mercador mercadillo) {
        PuestosMercadillos.put(mercadillo.getIDMercadillo(), mercadillo);
    }

    public static Map<Integer, ArrayList<Mercador.ObjetoMercadillo>> getMisObjetos(int cuentaID) {
        if (ObjMercadillos.get(cuentaID) == null) {
            ObjMercadillos.put(cuentaID, new HashMap());
        }
    ...
    public static Personagens getCasado(int id) {
        return Esposos.get(id);
    }

    public static synchronized void addEsposo(int id, Personagens perso) {
        Personagens esposo = Esposos.get(id);
        if (esposo != null) {
    ...
    public static void discursoSacerdote(Personagens perso, Maps mapa, int idSacerdote) {
        Personagens esposo = Esposos.get(0);
        Personagens esposa = Esposos.get(1);
        ...
        SocketManager.ENVIAR_cMK_CHAT_MENSAJE_MAPA(perso.getMapa(), "", -1, "Padre", String.valueOf(perso.getNombre()) + " Voc\u00ea aceita " + World.getCasado(perso.getSexo() == 1 ? 0 : 1).getNombre() + " em casamento ?");
    ...
    public static void casando(Personagens hombre, Personagens mujer, int isOK) {
        if (isOK > 0) {
    ...
    public static Collection<Set_Vivo> getTodosObjevivos() {
        return Objevivos.values();
    }

    public static Set_Vivo getObjevivos(int idObjevivo) {
        return Objevivos.get(idObjevivo);
    }

    public static synchronized void addObjevivo(Set_Vivo objevivo) {
        Objevivos.put(objevivo.getID(), objevivo);
    }

    public static Animacao getAnimacion(int animacionId) {
        return Animaciones.get(animacionId);
    }

    public static void addAnimation(Animacao animacion) {
        Animaciones.put(animacion.getId(), animacion);
    }

    public static void addReto(Desafios reto) {
        Retos.put(reto.getId(), reto);
    }

    public static Desafios getReto(int id) {
        return Retos.get(id);
    }

    public static synchronized Stockage nuevaTienda(int objetoID, int precio, int cantidad) {
        return new Stockage(objetoID, precio, cantidad);
    }

    public static synchronized void agregarTienda(Stockage tiendas, boolean salvarSQL) {
    ...
    public static Stockage getObjTienda(int id) {
    ...
    public static Pets getMascota(int id) {
        return Mascotas.get(id);
    }

    public static synchronized void addMascota(Pets mascota) {
        Mascotas.put(mascota.getID(), mascota);
    }

    public static Collection<Pets> getTodasMascotas() {
        return Mascotas.values();
    }

    public static void agregarMascotaModelo(int id, Pets.MascotaModelo mascota) {
        MascotasModelos.put(id, mascota);
    }

    public static Pets.MascotaModelo getMascotaModelo(int id) {
    ...
    public static synchronized void addCercado(Maps.Cercado cercado) {
        Cercados.put(cercado.getMapa().getID(), cercado);
    }

    public static Maps.Cercado getCercadoPorMapa(short mapa) {
        return Cercados.get(mapa);
    }

    public static Collection<Maps.Cercado> todosCercados() {
    ...
    public static void addRecaudador(Coletor recauda) {
    ...
    public static Coletor getRecaudador(int id) {
    ...
    public static Map<Integer, Coletor> getTodosRecaudadores() {
        return Recaudadores;
    }

    public static Coletor getRecauPorMapaID(short id) {
        for (Map.Entry<Integer, Coletor> recau : Recaudadores.entrySet()) {
    ...
    public static void addRankingPVP(RankPVP rank) {
    ...
    public static RankPVP getRanking(int id) {
    ...
    public static String nombreLiderRankingPVP() {
        ...
        int vict = 0;
        int derr = 0;
        for (RankPVP rank : RankingsPVP.values()) {
    ...
    public static String prismasGeoposicion(int alineacion) {
        ...
        for (SubArea subarea : SubAreas.values()) {
            ...
            str = World.getPrisma(subarea.getPrismaID()) == null ? String.valueOf(str) + "0,1" : String.valueOf(str) + (subarea.getPrismaID() == 0 ? 0 : (int)World.getPrisma(subarea.getPrismaID()).getMapaID()) + ",1";
            primero = true;
            ++subareas;
        }
        if (alineacion == 1) {
    ...
    public static void addKoliseo1(Personagens perso) {
        if (Koliseo1.isEmpty()) {
            Koliseo1.add(perso);
        } else {
            for (Personagens persos : Koliseo1) {
    ...
    public static void addKoliseo2(Personagens perso) {
        if (Koliseo2.isEmpty()) {
            Koliseo2.add(perso);
        } else {
            for (Personagens persos : Koliseo2) {
                if (persos == null || persos.getNivel() < perso.getNivel()) continue;
    ...
    public static void addKoliseo3(Personagens perso) {
        if (Koliseo3.isEmpty()) {
            Koliseo3.add(perso);
        } else {
            for (Personagens persos : Koliseo3) {
                if (persos == null || persos.getNivel() < perso.getNivel()) continue;
    ...
    public static void crearGruposKoliseo1() {
        CopyOnWriteArrayList<Personagens> kolis1 = new CopyOnWriteArrayList<Personagens>();
        for (Personagens persos : Koliseo1) {
        ...
        for (int i = 0; i < size; i += 3) {
            Personagens koli1 = null;
            Personagens koli2 = null;
            Personagens koli3 = null;
            Random rand = new Random();
            int random = rand.nextInt(kolis1.size() - 1);
            koli1 = (Personagens)kolis1.get(random);
            kolis1.remove(random);
            random = rand.nextInt(kolis1.size() - 1);
            koli2 = (Personagens)kolis1.get(random);
            kolis1.remove(random);
            random = rand.nextInt(kolis1.size() - 1);
            koli3 = (Personagens)kolis1.get(random);
            kolis1.remove(random);
            if (koli1 == null || koli2 == null || koli3 == null) continue;
            Personagens.GrupoKoliseo grupo = new Personagens.GrupoKoliseo(koli1, koli2, koli3, 1);
    ...
    public static void addEncarnacion(Incarnacao encarnacion) {
        Encarnaciones.put(encarnacion.getID(), encarnacion);
    }

    public static Incarnacao getEncarnacion(int id) {
    ...
    public static class Area {
        ...
        public Area(short id, int superArea, String nombre, int alineacion, int prisma) {
            ...
            if (this._superArea == null) {
                ...
                World.addSuperArea(this._superArea);
            }
            this._alineacion = 0;
            this._prisma = 0;
        ...
        public ArrayList<Maps> getMapas() {
            ArrayList<Maps> mapas = new ArrayList<Maps>();
    ...
    public static class Intercambio {
        private Personagens _perso1;
        private Personagens _perso2;
        ...
        public Intercambio(Personagens p1, Personagens p2) {
            this._perso1 = p1;
            this._perso2 = p2;
    ...
    public static class InvitarTaller {
        private Personagens _artesano1;
        private Personagens _cliente2;
        private PrintWriter _out1;
        private PrintWriter _out2;
        ...
        public InvitarTaller(Personagens p1, Personagens p2, int max) {
            this._artesano1 = p1;
            this._cliente2 = p2;
        ...
        public void aplicar() {
            Profissao.AccionTrabajo trabajo = this._artesano1.getHaciendoTrabajo();
            boolean resultado = trabajo.iniciarTrabajoPago(this._artesano1, this._cliente2, this._objArtesano1, this._objCliente2);
            Profissao.StatsOficio oficio = this._artesano1.getOficioPorTrabajo(trabajo.getIDTrabajo());
            if (oficio != null) {
                Objeto nuevoObj;
                Objeto obj;
                int cant;
    ...
    public static class ObjetoSet {
        ...
        private ArrayList<Personagens.Stats> _bonus = new ArrayList();

        public ObjetoSet(int id, String items, String bonuses) {
            this._id = id;
            for (String str : items.split(",")) {
                try {
                    Objeto.ObjetoModelo t = World.getObjModelo(Integer.parseInt(str.trim()));
                    if (t == null) continue;
            ...
            this._bonus.add(new Personagens.Stats());
            for (String str : bonuses.split(";")) {
                Personagens.Stats S = new Personagens.Stats();
        ...
        public Personagens.Stats getBonusStatPorNroObj(int numero) {
            if (numero > this._bonus.size()) {
                return new Personagens.Stats();
            }
    ...
    public static class SubArea {
        ...
        private int _alineacion;
        private String _nombre;
        private ArrayList<Maps> _mapas = new ArrayList();
        ...
        public SubArea(int id, short areaID, int alineacion, String nombre, int conquistable, int prisma) {
            this._id = id;
            this._nombre = nombre;
            this._area = World.getArea(areaID);
            this._alineacion = 0;
    ...
    public static class Trueque {
        ...
        public Trueque(Personagens perso, String Objetopedir, String Objetodar) {
            this._perso = perso;
        ...
        public synchronized void aplicar() {
            ...
            for (Duo<Integer, Integer> duo : this._objetos) {
                ...
                Objeto obj = World.getObjeto(idObj);
            ...
            if (this._resucitar) {
                if (this._idMascota != -1) {
                    Objeto objMasc = World.getObjeto(this._idMascota);
            ...
            } else if (this._objetoConseguir != -1 && this._cantObjConseguir != -1 && !this._perso.addObjetoSimilar(nuevoObjeto = World.getObjModelo(this._objetoConseguir).crearObjDesdeModelo(this._cantObjConseguir, false), true, -1)) {
                World.addObjeto(nuevoObjeto, true);
        ...
        public synchronized void addObjetoTrueque(int idObjeto, int cantObj) {
            ...
            if (duo != null) {
                ...
                block0: for (Duo<Integer, Integer> acouple : this._objetos) {
                    Objeto.ObjetoModelo objModelo = World.getObjeto((Integer)acouple._primero).getModelo();
                ...
                if (cantSolicitadas > 0 && pedir.length == j) {
                    if (this._resucitar) {
                        if (this._idMascota != -1) {
                            Objeto mascota = World.getObjeto(this._idMascota);
                        ...
                        SocketManager.ENVIAR_EmK_MOVER_OBJETO_DISTANTE(this._out, 'O', "+", "1|" + cantFinal + "|" + idObjModDar + "|" + World.getObjModelo(idObjModDar).getStringStatsObj());
