...
public class Pathfinding {
	...
	public static Combate.Luchador getEnemigoAlrededor(short celdaID, Mapa mapa, Combate pelea) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (sigCelda == null || (luchador = sigCelda.getPrimerLuchador()) == null
					|| luchador.getEquipoBin() == pelea.getLuchadorTurno().getEquipoBin())
				continue;
			return luchador;
		}
		return null;
	...
	public static boolean hayAlrededor(Mapa mapa, Combate pelea, Combate.Luchador l, boolean amigo) {
		...
		for (int i = 0; i < n; ++i) {
			Combate.Luchador luchador;
			char dir = arrc[i];
			Mapa.Celda sigCelda = mapa
					.getCelda(Pathfinding.getSigIDCeldaMismaDir(l.getCeldaPelea().getID(), dir, mapa, false));
			if (sigCelda == null || (luchador = sigCelda.getPrimerLuchador()) == null
					|| !(amigo ? luchador.getEquipoBin() == l.getEquipoBin()
							: luchador.getEquipoBin() != l.getEquipoBin()))
				continue;
			return true;
		}
		return false;
	...
	public static boolean esSiguienteA(int celda1, int celda2, Mapa mapa) {
		...
		return celda1 + (ancho - 1) == celda2 || celda1 + ancho == celda2 || celda1 - (ancho - 1) == celda2
				|| celda1 - ancho == celda2;
	}
	...
	public static short getNuevaCeldaDespuesGolpe(Mapa mapa, Mapa.Celda celdaInicio, Mapa.Celda celdaObjetivo,
			int valor, Combate pelea, Combate.Luchador objetivo) {
	...
	public static short getCeldaDespEmpujon(Mapa mapa, Mapa.Celda celdaLanz, Mapa.Celda celdaObje, int valor,
			Combate pelea, Combate.Luchador objetivo) {
	...
	public static ArrayList<Combate.Luchador> getObjetivosZonaArma(Combate pelea, int tipo, Mapa.Celda celda,
			short celdaIDLanzador) {
		...
		case 7: {
			...
			if ((g = Pathfinding.getPrimerLuchadorMismaDireccion(pelea.getMapaCopia(), celdaIDLanzador,
					(char) (c - '\u0001'))) != null) {
				objetivos.add(g);
			}
			if ((h = Pathfinding.getPrimerLuchadorMismaDireccion(pelea.getMapaCopia(), celdaIDLanzador,
					(char) (c + '\u0001'))) != null) {
				objetivos.add(h);
			}
			if ((i = celda.getPrimerLuchador()) == null)
				break;
			objetivos.add(i);
			break;
		}
		case 4: {
			Combate.Luchador l;
			Combate.Luchador k;
			Combate.Luchador j = Pathfinding.getPrimerLuchadorMismaDireccion(pelea.getMapaCopia(), celdaIDLanzador,
					(char) (c - '\u0001'));
			if (j != null) {
				objetivos.add(j);
			}
			if ((k = Pathfinding.getPrimerLuchadorMismaDireccion(pelea.getMapaCopia(), celdaIDLanzador,
					(char) (c + '\u0001'))) != null) {
				objetivos.add(k);
			}
			if ((l = celda.getPrimerLuchador()) == null)
				break;
			objetivos.add(l);
			break;
		}
	...
	private static Combate.Luchador getLuchadorAntesCelda(short celdaID, char dir, Mapa mapa) {
		short nueva2CeldaID = Pathfinding
				.getSigIDCeldaMismaDir(Pathfinding.getSigIDCeldaMismaDir(celdaID, dir, mapa, false), dir, mapa, false);
	...
	public static char getDirEntreDosCeldas(short celdaID1, short celdaID2, Mapa mapa, boolean combate) {
		...
		for (Iterator<Character> iterator = direcciones.iterator(); iterator.hasNext();) {
	...
	public static ArrayList<Mapa.Celda> getCeldasAfectadasEnElArea(Mapa mapa, short celdaID, short celdaIDLanzador,
		...
		case 'C': {
			for (int a = 0; a < tama\u00f1o; ++a) {
				...
				for (Mapa.Celda aCell : cases2) {
					...
					for (int i = 0; i < n; ++i) {
						char d = arrc[i];
						Mapa.Celda cell = mapa
								.getCelda(Pathfinding.getSigIDCeldaMismaDir(aCell.getID(), d, mapa, true));
						if (cell == null || cases.contains(cell))
							continue;
						cases.add(cell);
					}
				}
	...
	public static short getCeldaMasCercanaAlrededor(Mapa mapa, short celdaInicio, short celdaFinal,
		...
		int i;
		char[] arrayOfChar1;
		for (i = (arrayOfChar1 = dirs).length, b = 0; b < i;) {
			char d = arrayOfChar1[b];
			short sigCelda = getSigIDCeldaMismaDir(celdaInicio, d, mapa, true);
			Celda C = mapa.getCelda(sigCelda);
			if (C == null) {
				break;
			}
			int dis = distanciaEntreDosCeldas(mapa, celdaFinal, sigCelda);
	...
	public static short getCeldaMasCercanaAlrededor2(Mapa mapa, short celdaInicio, short celdaFinal, int alcanceMin,
			int alcanceMax) {
		Mapa.Celda C;
		short dist = 1000;
		short celdaID = celdaInicio;
		...
		while (i < alcanceMax) {
			...
			if (++i > alcanceMin && (C = mapa.getCelda(sigCelda)) != null && C.esCaminable(true)
					&& C.getPrimerLuchador() == null)
				break;
		}
	...
	public static ArrayList<Mapa.Celda> pathMasCortoEntreDosCeldas(Mapa mapa, short inicio, short destino,
			int distMax) {
	...
	public static ArrayList<Short> listaCeldasPorDistancia(Combate pelea, Luchador luchador, ArrayList<Short> celdas) {
		ArrayList<Short> celdasPelea = new ArrayList<Short>(), copiaCeldas = new ArrayList<Short>();
	...
	public static short celdaMovPerco(Mapa mapa, short celda) {
		...
		for (int i = 0; i < dir.length; ++i) {
			if (celda + dir[i] <= 14 && celda + dir[i] >= 464
					|| !mapa.getCelda((short) (celda + dir[i])).esCaminable(false))
				continue;
