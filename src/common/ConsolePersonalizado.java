...
public class ConsolePersonalizado implements Runnable {
    private Thread _thread;
    private Personagens _perso;

    public ConsolePersonalizado() {
    ...
    public void Comandos(String command) {
        ...
        if (fct.equals("LIMPAR")) {
            ...
            ConsolePersonalizado.sendEcho("<Msg ALL:> " + announce);
        } else if (fct.equalsIgnoreCase("QUEMON")) {
            String str = "==========\nLista de Jogadores ON:";
            ConsolePersonalizado.sendInfo(str);
            ...
            if (mapaID == -1 || celdaID == -1 || World.getMapa(mapaID) == null) {
                ...
                ConsolePersonalizado.sendEcho(str);
                return;
            }
            ...
            Personagens objetivo2 = this._perso;
            if (!(args.length <= 3 || (objetivo2 = World.getPjPorNombre(args[3])) != null && objetivo2.getPelea() == null && objetivo2.enLinea())) {
                String str = "O personagem n\u00e3o existe ou est\u00e1 em combate.";
                ConsolePersonalizado.sendEcho(str);
                return;
            }
            objetivo2.teleport(mapaID, celdaID);
            ConsolePersonalizado.sendEcho("O jogador " + objetivo2.getNombre() + " foi teleportado.");
        } else if (fct.equalsIgnoreCase("KICK")) {
            Personagens objetivo = this._perso;
            String nombre2 = null;
            try {
                nombre2 = args[1];
            }
            ...
            objetivo = World.getPjPorNombre(nombre2);
            if (objetivo == null || !objetivo.enLinea()) {
                ConsolePersonalizado.sendEcho("O personagem n\u00e3o existe ou est\u00e1 em combate.");
                return;
            }
            if (objetivo.enLinea()) {
                objetivo.getCuenta().getEntradaPersonaje().salir();
                ConsolePersonalizado.sendEcho("Kick: " + objetivo.getNombre());
            }
        } else {
            if (fct.equalsIgnoreCase("HEROICO")) {
                ...
                ConsolePersonalizado.sendEcho("Modo heroico: " + nombre.equalsIgnoreCase("true"));
            ...
            if (fct.equals("?") || command.equals("HELP")) {
                ConsolePersonalizado.sendInfo("------------Commandos:------------");
                ConsolePersonalizado.sendInfo("- EXIT = Reloga servidor.");
                ConsolePersonalizado.sendInfo("- SALVAR = Salva o servidor.");
                ConsolePersonalizado.sendInfo("- INFO = Mostra informacoes do servidor.");
                ConsolePersonalizado.sendInfo("- QUEMON = Mostra jogadores on.");
                ConsolePersonalizado.sendInfo("- HEROIC = Ativa/desativa modo heroico.");
                ConsolePersonalizado.sendInfo("- ANTIDDOS = Ativa/desativa anti ddos");
                ConsolePersonalizado.sendInfo("- FULLHDV = Ativa HDV.");
                ConsolePersonalizado.sendInfo("- LOCK = Abre o servidor.");
                ConsolePersonalizado.sendInfo("- BLOCK = Tranca o servidor...");
                ConsolePersonalizado.sendInfo("- KICK = Kicka um jogador.");
                ConsolePersonalizado.sendInfo("- LIMPAR = Limpa o console.");
                ConsolePersonalizado.sendInfo("- PRESENTEALL = Da presente para todos do servidor online.");
                ConsolePersonalizado.sendInfo("- TELEPORT = Teleporta um jogador.");
                ConsolePersonalizado.sendInfo("- ANUNCIAR = Mensagem para o servidor.");
                ConsolePersonalizado.sendInfo("----------------------------------");
                ...
                ConsolePersonalizado.sendEcho("Server Bloqueado para GM : " + accesoGM);
                if (botarRango > 0) {
                    for (Personagens pj : World.getPJsEnLinea()) {
                        if (pj.getCuenta().getRango() >= accesoGM) continue;
                        pj.getCuenta().getEntradaPersonaje().salir();
                    }
                    ConsolePersonalizado.sendEcho("Jogadores com GM LVL inferior a " + accesoGM + " foram expulsos !");
                }
                ...
                ConsolePersonalizado.sendEcho("O item " + regalo + " foi entregue a todos os jogadores online.");
                ...
                if (estado == 1) {
                    ConsolePersonalizado.sendEcho("Server Aberto.");
                } else if (estado == 0) {
                    ConsolePersonalizado.sendEcho("Server Fechado.");
                } else if (estado == 2) {
                    ConsolePersonalizado.sendEcho("Server Salvando.");
                }
                ...
                ConsolePersonalizado.sendEcho("Le contre ddos est : " + valor);
                ...
                try {
                    ...
                    ConsolePersonalizado.sendEcho(message);
                }
                catch (Exception exception) {}
            } else {
                ConsolePersonalizado.sendError("Comando nao reconhecido ou incompleto.");
            }
        }
    }
    ...
    private void fullHdv(int deCadaModelo) {
        ConsolePersonalizado.sendEcho("Arranque de ofertas !");
        Objeto objeto = null;
        ...
        for (Objeto.ObjetoModelo objModelo : World.getObjModelos()) {
            try {
                ...
                for (int j = 0; j < deCadaModelo; ++j) {
                    if (objModelo.getTipo() != 85 && (hdv = this.getHdv((objeto = objModelo.crearObjDesdeModelo(1, false)).getModelo().getTipo())) >= 0) {
                        ...
                        objMercadillo = new Mercador.ObjetoMercadillo(this.calcularPrecio(objeto, cantidad), cantidad, -1, objeto);
            ...
            ConsolePersonalizado.sendEcho(String.valueOf((System.currentTimeMillis() - time1) / 1000L) + "sec modelo: " + objModelo.getID());
        }
        ConsolePersonalizado.sendEcho("Carregado em " + (System.currentTimeMillis() - time1) + "ms");
