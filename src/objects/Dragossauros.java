...
public class Dragossauros {
    ...
    private Timer _aumentarFecundo = new Timer(60000, new ActionListener(){
        ...
        public void actionPerformed(ActionEvent e) {
            Dragossauros dragossauros = Dragossauros.this;
            dragossauros._fecundadaHace = dragossauros._fecundadaHace + 1;
            Dragossauros.this.startFecundo();
        }
    });
    private String _vip = "";

    public Dragossauros(int color, int due\u00f1o) {
        int sexo = Fórmulas.getRandomValor(0, 1);
        this._id = World.getSigIDMontura();
        this._colorID = color;
        this._sexo = sexo;
        this._nivel = 100;
        ...
        World.addDragopavo(this);
    }

    public Dragossauros(int color, Dragossauros madre, Dragossauros padre) {
        int[] arrn = new int[4];
        arrn[2] = 1;
        int[] sexo = arrn;
        this._id = World.getSigIDMontura();
    ...
    public Dragossauros(int id, int color, int sexo, int amor, int resistencia, int nivel, long exp, String nombre, int fatiga, int energia, int reprod, int madurez, int serenidad, String objetos, String anc, String habilidad, int talla, short celda, short mapa, int due\u00f1o, int orientacion, int fecundable, int pareja, String vip) {
        this._id = id;
        this._colorID = color;
        this._sexo = sexo;
        this._amor = amor;
        ...
        for (String str : objetos.split(";")) {
            try {
                Objeto obj = World.getObjeto(Integer.parseInt(str));
    ...
    public void addObjAMochila(int id, int cant, Personagens perso) {
        Objeto objetoAgregar = World.getObjeto(id);
        ...
        if (objIgualEnMochila == null) {
            if (nuevaCant <= 0) {
                ...
                World.addObjeto(objIgualEnMochila, true);
            ...
            String str = "O+" + objIgualEnMochila.getID() + "|" + objIgualEnMochila.getCantidad() + "|" + objIgualEnMochila.getModelo().getID() + "|" + objIgualEnMochila.convertirStatsAString();
            World.eliminarObjeto(objetoAgregar.getID());
            SocketManager.ENVIAR_EsK_MOVER_A_TIENDA_COFRE_BANCO(perso, str);
            SocketManager.ENVIAR_OR_ELIMINAR_OBJETO(perso, id);
        } else {
            objetoAgregar.setCantidad(nuevaCant);
    ...
    public void removerDeLaMochila(int id, int cant, Personagens perso) {
        Objeto objARetirar = World.getObjeto(id);
        ...
        if (objIgualInventario == null) {
            if (nuevaCant <= 0) {
                ...
                World.addObjeto(objIgualInventario, true);
            ...
            World.eliminarObjeto(objARetirar.getID());
            String str = "O-" + id;
            SocketManager.ENVIAR_EsK_MOVER_A_TIENDA_COFRE_BANCO(perso, str);
        } else {
            objARetirar.setCantidad(nuevaCant);
    ...
    private String parseXpString() {
        return String.valueOf(this._experiencia) + "," + World.getExpNivel((int)this._nivel)._montura + "," + World.getExpNivel((int)(this._nivel + 1))._montura;
    }
    ...
    public void addXp(long aumentar) {
        ...
        while (this._experiencia >= (long)World.getExpNivel((int)(this._nivel + 1))._montura && this._nivel < 100) {
            this.subirNivel();
        }
    }
    ...
    public synchronized String getCriarMontura(Maps.Cercado cercado) {
        String str = "GM|+";
        ...
        str = World.getPersonaje(this._due\u00f1o) == null ? String.valueOf(str) + "Sin Due\u00f1o" : String.valueOf(str) + World.getPersonaje(this._due\u00f1o).getNombre();
        str = String.valueOf(str) + ";" + this._nivel + ";" + this._colorID;
        return str;
    }

    public synchronized void moverMontura(Personagens due\u00f1o, int casillas, boolean alejar) {
        int accion = 0;
        if (due\u00f1o == null) {
            return;
        }
