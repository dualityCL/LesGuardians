...
public class Gremio {
    ...
    public Gremio(int id, String nombre, String emblema, int nivel, long xp, int capital, int nroMaxRecau, String hechizos, String stats) {
        this._id = id;
        this._nombre = nombre;
        this._emblema = emblema;
        this._xp = xp;
    ...
    public void boostHechizo(int ID) {
        Hechizo.StatsHechizos SS = this._hechizos.get(ID);
        if (SS != null && SS.getNivel() == 5) {
            return;
        }
        this._hechizos.put(ID, SS == null ? Mundo.getHechizo(ID).getStatsPorNivel(1) : Mundo.getHechizo(ID).getStatsPorNivel(SS.getNivel() + 1));
    }

    public Personaje.Stats getStatsPelea() {
        return new Personaje.Stats(this._statsPelea);
    }
    ...
    public void addXp(long xp) {
        this._xp += xp;
        while (this._xp >= Mundo.getXPMaxGremio(this._nvl) && this._nvl < 200) {
            this.subirNivel();
        }
    }
    ...
    public String analizarInfoCercados() {
        ...
        for (Mapa.Cercado cercados : Mundo.todosCercados()) {
            ...
            for (Integer monturas : cercados.getListaCriando()) {
                Dragopavo montura = Mundo.getDragopavoPorID(monturas);
                if (montura == null) continue;
                if (primero) {
                    packet = String.valueOf(packet) + ",";
                }
                primero = true;
                packet = Mundo.getPersonaje(montura.getDue\u00f1o()) == null ? String.valueOf(packet) + montura.getColor() + "," + montura.getNombre() + ",SIN DUE\u00c3\u2018O" : String.valueOf(packet) + montura.getColor() + "," + montura.getNombre() + "," + Mundo.getPersonaje(montura.getDue\u00f1o()).getNombre();
            }
        }
        return packet;
    }
    ...
    public void decompilarHechizos(String strHechizo) {
        for (String split : strHechizo.split("\\|")) {
            ...
            this._hechizos.put(id, Mundo.getHechizo(id).getStatsPorNivel(nivel));
        }
    }
    ...
    public String compilarHechizo() {
        ...
        for (Map.Entry<Integer, Hechizo.StatsHechizos> statHechizo : this._hechizos.entrySet()) {
            if (!primero) {
                str = String.valueOf(str) + "|";
            }
    ...
    public String analizarRecauAGrmio() {
        String packet = String.valueOf(this.getNroRecau()) + "|" + Mundo.cantRecauDelGremio(this.getID()) + "|" + 100 * this.getNivel() + "|" + this.getNivel() + "|" + this.getStats(158) + "|" + this.getStats(176) + "|" + this.getStats(124) + "|" + this.getNroRecau() + "|" + this.getCapital() + "|" + (1000 + 10 * this.getNivel()) + "|" + this.compilarHechizo();
        return packet;
    }
    ...
    public static class MiembroGremio {
        ...
        public MiembroGremio(int id, Gremio gremio, String nombre, int nivel, int gfx, int r, long x, byte pXp, int derechos, String ultConeccion) {
            this._id = id;
            this._gremio = gremio;
            this._nombre = nombre;
            this._nivel = nivel;
        ...
        public String getVerdaderoNombre() {
            Personaje perso = Mundo.getPersonaje(this._id);
            if (perso == null) {
                return this._nombre;
            }
        ...
        public Personaje getPerso() {
            return Mundo.getPersonaje(this._id);
        }
