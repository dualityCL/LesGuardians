...
public class Fight {
    ...
    private Map<Integer, Personagens> _espectadores = new TreeMap<Integer, Personagens>();
    private Maps _mapaCopia;
    private Maps _mapaReal;
    ...
    private Map<Integer, Maps.Celda> _posinicial = new TreeMap<Integer, Maps.Celda>();
    ...
    public Map<Integer, Maps.Celda> getPosInicial() {
    ...
    public Fight(short id, Maps mapa, Personagens init1, Personagens init2, byte tipo) {
        try {
            Random equipos;
            this._tipo = tipo;
            this._id = id;
            ...
            if (this._tipo != 0) {
                this._tiempoTurno = new Timer(1000, new ActionListener(){
                    ...
                    public void actionPerformed(ActionEvent e) {
                        if (Fight.this._tiempoInicio == 0L) {
                            Fight.this.tiempoTurno();
                        } else {
                            Fight.this._tiempoTurno.stop();
                        }
                    }
                });
    ...
    public Fight(short id, Maps mapa, Personagens perso, MOB_tmpl.GrupoMobs grupoMob, byte tipo) {
        this._tipo = tipo;
        this._id = id;
        this._mapaCopia = mapa.copiarMapa();
        this._mapaReal = mapa;
        ...
        this._equipo1.put(id1, this._luchInit1);
        for (Map.Entry<Integer, MOB_tmpl.MobGrado> entry : grupoMob.getMobs().entrySet()) {
        ...
        for (Map.Entry entry : equipo2) {
            ...
            Maps.Celda celdaRandom = this.getCeldaRandom(this._celdasPos2);
            if (celdaRandom == null) {
    ...
    public Fight(short id, Maps mapa, Personagens perso, Coletor recaudador) {
    ...
    public Fight(short id, Maps mapa, Personagens perso, Prisma prisma) {
    ...
    public Fight(short id, Maps mapa, ArrayList<Personagens> grupo1, ArrayList<Personagens> grupo2) {
        Maps mapaTemp;
        this._tipo = (byte)6;
        this._id = id;
        this._mapaCopia = mapa.copiarMapa();
    ...
    private Maps.Celda getCeldaRandom(List<Maps.Celda> celdas) {
        Maps.Celda celda;
        Random rand = new Random();
        CopyOnWriteArrayList<Maps.Celda> celdas2 = new CopyOnWriteArrayList<Maps.Celda>();
        ...
        int limit = 0;
        if (celdas2.size() == 1) {
            Maps.Celda celda2 = (Maps.Celda)celdas2.get(0);
        ...
        do {
            ...
            if ((size = celdas2.size()) == 1) {
                celda = (Maps.Celda)celdas2.get(0);
                continue;
            }
            int id = rand.nextInt(size - 1);
            celda = (Maps.Celda)celdas2.get(id);
    ...
    private ArrayList<Maps.Celda> analizarPosiciones(int num) {
    ...
    public boolean celdaOcupada(short celda) {
        Maps.Celda celd = this._mapaCopia.getCelda(celda);
        if (celd == null) {
            return true;
        }
    ...
    private boolean grupoCeldasContiene(ArrayList<Maps.Celda> celdas, int celda) {
    ...
    public void unirsePelea(Personagens perso, int idOtroPerso) {
        Luchador luchador;
        Personagens.Grupo g;
        Maps.Celda celda;
    ...
    public void desconectarLuchador(Personagens perso) {
        Luchador luchador = this.getLuchadorPorPJ(perso);
        if (luchador == null) {
            return;
        }
    ...
    public void reconectarLuchador(Personagens perso) {
    ...
    public synchronized void botonBloquearEspect(int id) {
        ...
        if (this._espectadores.size() > 0) {
            for (Personagens espec : this._espectadores.values()) {
    ...
    public boolean puedeMoverseLuchador(Luchador movedor, GameThread.AccionDeJuego GA) {
        ...
        Personagens perso = movedor.getPersonaje();
    ...
    public void finalizarMovimiento(Personagens perso) {
    ...
    public void pasarTurno(Personagens perso) {
    ...
    private Timer resetFinTurno(final Luchador luch) {
        ActionListener accion = new ActionListener(){
            ...
            public void actionPerformed(ActionEvent event) {
                Fight.this._esperarTempAccion.stop();
                Fight fight = Fight.this;
                fight._cantTimer = fight._cantTimer + 1;
                Fight.this.finTurno(luch);
            }
        };
    ...
    public synchronized void finTurno(Luchador luch) {
        ...
        try {
            ...
            if (!luchador._estaRetirado) {
                this._tempAccion = "";
                if (!luchador._estaMuerto) {
                    for (SpellEffect EH : luchador.getBuffsPorEfectoID(131)) {
                        int pDa\u00f1os;
                        ...
                        Personagens.Stats totalLanz = EH.getLanzador().getTotalStatsConBuff();
                        int inte = 0;
                    ...
                    this._tempLuchadorPAusados = 0;
                    this._tempLuchadorPMusados = 0;
                    Personagens.Stats statsConBuff = luchador.getTotalStatsConBuff();
    ...
    public int intentarLanzarHechizo(Luchador lanzador, Spell.StatsHechizos sHechizo, short celdaID) {
        Personagens perso = lanzador.getPersonaje();
        ...
        Maps.Celda celda = this._mapaCopia.getCelda(celdaID);
                ...
                if ((this._tipo == 4 || this._tipo == 3) && lanzador.getPersonaje() != null) {
                    ...
                    for (Map.Entry entry : copiaRetos.entrySet()) {
                        ...
                        switch (reto) {
                            ...
                            case 18: {
                                for (SpellEffect efecto : sHechizo.getEfectos()) {
    ...
    public void intentarCaC(Personagens perso, short celdaID) {
        ...
        if (perso.getObjPosicion(1) == null) {
            ...
            if (objetivo != null) {
                ...
                if ((da\u00f1oFinal = SpellEffect.aplicarBuffContraGolpe(da\u00f1oFinal, objetivo, lanzador, this, -1, false)) > objetivo.getPDVConBuff()) {
            ...
            if (esFC) {
                ...
                ArrayList<SpellEffect> efectos = arma.getEfectos();
                if (esGC) {
                    efectos = arma.getEfectosCriticos();
                }
                for (SpellEffect EH : efectos) {
                    if (this._estadoPelea != 3) break;
    ...
    public boolean puedeLanzarHechizo(Luchador lanzador, Spell.StatsHechizos sHechizo, Maps.Celda celda, short celdaObjetivo) {
        boolean modif;
        int modi;
        ...
        Personagens.Stats statsConBuff = lanzador.getTotalStatsConBuff();
        if (lanzador.getTipo() == 1 && perso.getHechizosSetClase().containsKey(sHechizo.getHechizoID())) {
            boolean modif2;
    ...
    public String getPanelResultados(int equipoGanador) { //FIXME
        ...
        for (Luchador luchador : ganadores) {
            ...
            if (this._tipo == 1 && pj1 != null) {
                if (!World.estaRankingPVP(pj1.getID())) {
                    RankPVP rank = new RankPVP(pj1.getID(), pj1.getNombre(), 0, 0, pj1.getAlineacion());
                    World.addRankingPVP(rank);
                    SQLManager.INSERT_RANKINGPVP(rank);
                    rank.aumentarVictoria();
                } else {
                    World.getRanking(pj1.getID()).aumentarVictoria();
                }
            }
            if (this._tipo == 1 && pj1 != null && this._misionPVP > 0) {
                if (this._equipo1.containsValue(luchador) && this._misionPVP == 1) {
                    ...
                    if ((kamas = pj1.getMisionPVP().getKamasRecompensa()) > 0) {
                        ...
                        Objeto nuevo2 = World.getObjModelo(10275).crearObjDesdeModelo(LesGuardians.CHAPAS_MISION, false);
                        if (!pj1.addObjetoSimilar(nuevo2, true, -1)) {
                            World.addObjeto(nuevo2, true);
                ...
                pj1.addExp(exp);
                SocketManager.ENVIAR_Im_INFORMACION(pj1, "08;" + exp);
                nuevo = World.getObjModelo(10275).crearObjDesdeModelo(LesGuardians.CHAPAS_MISION, false);
            ...
            if (this._tipo == 4 && pj1 != null) {
                float porc = World.getBalanceMundo(pj1.getAlineacion()) / 100.0f;
            ...
            if (!pj1.addObjetoSimilar(nuevo = World.getObjModelo(11158).crearObjDesdeModelo(1, false), true, -1)) {
                World.addObjeto(nuevo, true);
                pj1.addObjetoPut(nuevo);
                SocketManager.ENVIAR_OAKO_APARECER_OBJETO(pj1, nuevo);
            }
        ...
        for (Luchador luchador : perdedores) {
            ...
            Personagens pj2 = luchador.getPersonaje();
            ...
            if (!World.estaRankingPVP(pj2.getID())) {
                RankPVP rank = new RankPVP(pj2.getID(), pj2.getNombre(), 0, 0, pj2.getAlineacion());
                World.addRankingPVP(rank);
                SQLManager.INSERT_RANKINGPVP(rank);
                rank.aumentarDerrota();
                continue;
            }
            World.getRanking(pj2.getID()).aumentarDerrota();
        }
        int grupoPP = 0;
        long minkamas = 0L;
        long maxkamas = 0L;
        ...
        ArrayList<World.Drop> posibleDrops = new ArrayList<World.Drop>();
        if (LesGuardians.MODO_HEROICO && this._tipo != 0 && !LesGuardians._cerrando) {
            block14: for (Luchador perdedor : perdedores) {
                ...
                Personagens pjPerd = perdedor.getPersonaje();
                if (mobGrado != null) {
                    this._objetosRobados.addAll(((MOB_tmpl.MobGrado)((Object)mobGrado)).getObjetoHeroico());
                    ((MOB_tmpl.MobGrado)((Object)mobGrado)).borrarObjetosHeroico();
                    this._kamasRobadas += ((MOB_tmpl.MobGrado)((Object)mobGrado)).getKamasHeroico() / 2L;
                    ((MOB_tmpl.MobGrado)((Object)mobGrado)).borrarKamasHeroico();
                    continue;
                }
        ...
        if ((this._tipo == 4 || this._tipo == 5) && equipoGanador == 1) {
            ...
            for (Luchador ganador : ganadores) {
                ...
                Personagens pjGanador = ganador.getPersonaje();
                if (pjGanador != null) {
                    Pets pets;
                    ...
                    if (this._tipo == 4 && (pets = pjGanador.getMascota()) != null && pets.esDevoraAlmas()) {
                        ...
                        if (comio) {
                            SocketManager.ENVIAR_OCK_ACTUALIZA_OBJETO(pjGanador, World.getObjeto(pets.getID()));
                        }
                    }
                }
            ...
            if (exito) {
                for (Map.Entry<Integer, Integer> entry : this._retos.entrySet()) {
                    ...
                    if (exitoReto == 2) continue;
                    Desafios desafios = World.getReto(reto);
            ...
            for (Luchador perdedor : perdedores) {
                ...
                for (World.Drop drop : mOB_tmpl.getDrops()) {
                    ...
                    posibleDrops.add(new World.Drop(drop.getObjetoID(), 0, cant, drop.getDropMax()));
                }
            }
            ...
            if (this._esCapturable) {
                ...
                this._piedraAlma = new Almas(World.getSigIDObjeto(), 1, 7010, -1, piedraStats);
                for (Luchador ganador : ganadores) {
                ...
                if (this._capturadores.size() > 0 && !this._mapaReal.esArena()) {
                    ...
                    while (i < this._capturadores.size()) {
                        try {
                            ...
                            Personagens capturador = capt.getPersonaje();
                            Objeto objPos1 = capturador.getObjPosicion(1);
                            if (objPos1.getModelo().getTipo() != 83) {
                                this._capturadores.remove(capt);
                                continue;
                            }
                            World.Duo<Integer, Integer> piedraJug = Fórmulas.decompilarPiedraAlma(objPos1);
        ...
        for (Luchador ganador : ganadores) {
            ...
            if (tipoX == 0) {
                ...
                ArrayList<World.Drop> tempDrops = new ArrayList<World.Drop>();
                ...
                if (this._tipo == 5 && pjGanador != null) {
                        ...
                        if (collection.size() > 0) {
                            ...
                            for (Objeto objeto : collection) {
                                ...
                                if (pjGanador.addObjetoSimilar(objeto, true, -1)) {
                                    World.eliminarObjeto(objeto.getID());
                                } else {
                    ...
                    for (World.Drop drop : tempDrops) {
                        ...
                        Random random;
                        int cant;
                        OM = World.getObjModelo(drop.getObjetoID());
                    ...
                    if (this._tipo == 3) {
                        for (Luchador luchador : perdedores) {
                            int idMob;
                            MOB_tmpl.MobGrado mob = luchador.getMob();
                        ...
                        if (!pjGanador.addObjetoSimilar(this._piedraAlma, false, -1)) {
                            World.addObjeto(this._piedraAlma, true);
                    ...
                    for (Map.Entry entry : objetosGanados.entrySet()) {
                        OM = World.getObjModelo((Integer)entry.getKey());
                        if (OM == null) continue;
                        if (drops.length() > 0) {
                            drops = String.valueOf(drops) + ",";
                        }
                        ...
                        if (pjGanador == null) {
                            Personagens invocador = ganador.getInvocador().getPersonaje();
            ...
            if (pjGanador != null) {
                int maxHonor;
                packet.append("2;" + ganador.getID() + ";" + ganador.getNombreLuchador() + ";" + ganador.getNivel() + ";" + (ganador._estaMuerto ? "1" : "0") + ";" + (pjGanador.getAlineacion() != -1 ? World.getExpNivel((int)pjGanador.getNivelAlineacion())._pvp : 0) + ";" + pjGanador.getHonor() + ";");
                if (this._tipo == 1) {
                ...
                if ((maxHonor = World.getExpNivel((int)(pjGanador.getNivelAlineacion() + 1))._pvp) == -1) {
                    maxHonor = World.getExpNivel((int)pjGanador.getNivelAlineacion())._pvp;
                }
            ...
            packet.append("2;" + ganador.getID() + ";" + ganador.getNombreLuchador() + ";" + ganador.getNivel() + ";" + (ganador._estaMuerto ? "1" : "0") + ";" + World.getExpNivel((int)prisma.getNivel())._pvp + ";" + prisma.getHonor() + ";");
            n = World.getExpNivel((int)(prisma.getNivel() + 1))._pvp;
            if (n == -1) {
                n = World.getExpNivel((int)prisma.getNivel())._pvp;
            }
        ...
        for (Luchador perdedor : perdedores) {
            ...
            Personagens pjPerdedor = perdedor.getPersonaje();
            ...
            if (pjPerdedor != null) {
                int maxHonor;
                packet.append("0;" + perdedor.getID() + ";" + perdedor.getNombreLuchador() + ";" + perdedor.getNivel() + ";" + (perdedor._estaMuerto ? "1" : "0") + ";" + (pjPerdedor.getAlineacion() != -1 ? World.getExpNivel((int)pjPerdedor.getNivelAlineacion())._pvp : 0) + ";" + pjPerdedor.getHonor() + ";");
                if (this._tipo == 1) {
                    if (this._luchInit2.getPersonaje().getAlineacion() != 0 && this._luchInit1.getPersonaje().getAlineacion() != 0) {
                        if (this._luchInit2.getPersonaje().getCuenta().getActualIP().compareTo(this._luchInit1.getPersonaje().getCuenta().getActualIP()) != 0) {
                ...
                if ((maxHonor = World.getExpNivel((int)(pjPerdedor.getNivelAlineacion() + 1))._pvp) == -1) {
                    maxHonor = World.getExpNivel((int)pjPerdedor.getNivelAlineacion())._pvp;
                }
            ...
            packet.append("0;" + perdedor.getID() + ";" + perdedor.getNombreLuchador() + ";" + perdedor.getNivel() + ";" + (perdedor._estaMuerto ? "1" : "0") + ";" + World.getExpNivel((int)prisma.getNivel())._pvp + ";" + prisma.getHonor() + ";");
            int maxHonor = World.getExpNivel((int)(prisma.getNivel() + 1))._pvp;
            if (maxHonor == -1) {
                maxHonor = World.getExpNivel((int)prisma.getNivel())._pvp;
            }
        ...
        if (World.getRecauPorMapaID(this._mapaCopia.getID()) != null && this._tipo == 4) {
            Coletor recau = World.getRecauPorMapaID(this._mapaCopia.getID());
            ...
            packet.append("5;" + recau.getID() + ";" + recau.getN1() + "," + recau.getN2() + ";" + World.getGremio(recau.getGremioID()).getNivel() + ";0;");
            Guild gremio = World.getGremio(recau.getGremioID());
            packet.append(String.valueOf(gremio.getNivel()) + ";");
            packet.append(String.valueOf(gremio.getXP()) + ";");
            packet.append(String.valueOf(World.getXPMaxGremio(gremio.getNivel())) + ";");
    ...
    public boolean acaboPelea(boolean usarPara5y2, boolean e1Muerto) {
        ...
        if (equipo1Muerto || equipo2Muerto || !this.verificaSiQuedaUno()) {
            ...
            for (Personagens p : this._espectadores.values()) {
                p.retornoMapa();
            }
            ...
            for (Luchador ganador : ganadores) {
                ...
                if (ganador._recaudador != null) {
                    for (Personagens pj : World.getGremio(this._gremioID).getPjMiembros()) {
                        if (pj == null || !pj.enLinea()) continue;
                        SocketManager.ENVIAR_gITM_INFO_RECAUDADOR(pj, Coletor.analizarRecaudadores(pj.getGremio().getID()));
                    ...
                    for (Personagens z : World.getMapa(ganador._recaudador.getMapaID()).getPersos()) {
                    ...
                    for (Personagens z : World.getMapa(ganador._prisma.getMapaID()).getPersos()) {
            ...
            for (Luchador perdedor : perdedores) {
                ...
                if (perdedor._prisma != null) {
                    World.SubArea subarea = this._mapaReal.getSubArea();
                    for (Personagens z : World.getPJsEnLinea()) {
                        if (z == null) continue;
    ...
    public Luchador getLuchadorPorPJ(Personagens perso) {
    ...
    public void actualizarInfoJugador() {
        Personagens.Stats statConBuff = this._ordenJugadores.get(this._nroOrdenLuc).getTotalStatsConBuff();
     ...
     */
    public void retirarsePelea(Personagens retirador, Personagens expulsado) {
                        ...
                        if (lucExpulsado != null && puedeExpulsar) {
                            ...
                            if (this._tipo == 0) {
                                for (Luchador luch : this.luchadoresDeEquipo(3)) {
                                    Personagens perso = luch.getPersonaje();
     ...
     */
    public static void agregarEspadaDePelea(Maps mapa, Personagens perso) {
        block7: for (Map.Entry<Short, Fight> peleas : mapa.getPeleas().entrySet()) {
            Luchador luchador;
            Fight pelea = peleas.getValue();
            if (pelea._estadoPelea != 2) continue;
            Personagens persoInit1 = pelea._luchInit1.getPersonaje();
    ...
    public static short getPeleaIDPorLuchador(Maps mapa, int id) {
        for (Map.Entry<Short, Fight> pelea : mapa.getPeleas().entrySet()) {
    ...
    public static class Glifo {
        ...
        private Spell.StatsHechizos _glifoHechizo;
        private byte _duracion;
        private Fight _pelea;
        private int _color;

        public Glifo(Fight pelea, Luchador lanzador, Maps.Celda celda, byte tama\u00f1o, Spell.StatsHechizos glifoHechizo, byte duracion, int hechizo) {
            this._pelea = pelea;
    ...
    public static class HechizoLanzado {
        ...
        public HechizoLanzado(Luchador objetivo, Spell.StatsHechizos sHechizo, Luchador lanzador) {
    ...
    public static class Luchador {
        ...
        private Fight _pelea;
        private int _tipo = 0;
        private MOB_tmpl.MobGrado _mob = null;
        private Personagens _perso = null;
        private int _equipoBin = -2;
        private Maps.Celda _celda;
        private ArrayList<SpellEffect> _buffsPelea = new ArrayList();
        ...
        private Coletor _recaudador = null;
        private Prisma _prisma = null;
        private Personagens _doble = null;
        ...
        private Personagens.Stats _totalStats;
        ...
        public Luchador(Fight pelea, MOB_tmpl.MobGrado mob) {
            this._pelea = pelea;
            this._tipo = 2;
            this._mob = mob;
        ...
        public Luchador(Fight pelea, Personagens perso) {
            this._pelea = pelea;
            if (perso.esDoble()) {
                this._tipo = 10;
                this._doble = perso;
        ...
        public Luchador(Fight pelea, Coletor recaudador) {
            this._pelea = pelea;
            this._tipo = 5;
            this._recaudador = recaudador;
            this._id = -1;
            Guild gremio = World.getGremio(recaudador.getGremioID());
        ...
        public Luchador(Fight pelea, Prisma prisma) {
            this._pelea = pelea;
            this._tipo = 7;
            this._prisma = prisma;
            this._id = -1;
        ...
        public void addHechizoLanzado(Luchador objetivo, Spell.StatsHechizos sort, Luchador lanzador) {
        ...
        public ArrayList<SpellEffect> getBuffPelea() {
        ...
        public boolean calculaSiGC(int porcGC) {
            ...
            Personagens.Stats statsConBuff = this.getTotalStatsConBuff();
        ...
        public boolean testSiEsGC(int porcGC, Spell.StatsHechizos sHechizo, Luchador luchador) {
            int jet;
            Personagens perso = luchador.getPersonaje();
            if (porcGC < 2) {
                return false;
        ...
        public ArrayList<SpellEffect> getBuffsPorEfectoID(int efectotID) {
            ArrayList<SpellEffect> buffs = new ArrayList<SpellEffect>();
            for (SpellEffect buff : this._buffsPelea) {
        ...
        public Personagens.Stats getTotalStatsSinBuff() {
            Personagens.Stats stats = new Personagens.Stats(new TreeMap<Integer, Integer>());
            stats = this._totalStats;
            return stats;
        }

        public Personagens.Stats getTotalStatsConBuff() {
            Personagens.Stats stats = new Personagens.Stats(new TreeMap<Integer, Integer>());
            stats = this._totalStats;
            stats = Personagens.Stats.acumularStats(stats, this.getBuffsStatsPelea());
            return stats;
        }

        private Personagens.Stats getBuffsStatsPelea() {
            Personagens.Stats stats = new Personagens.Stats();
            for (SpellEffect entry : this._buffsPelea) {
        ...
        public String stringGM() {
            ...
            switch (this._tipo) {
                case 1: {
                    Personagens.Stats totalStats = this.getTotalStatsConBuff();
                ...
                case 5: {
                    ...
                    Guild G = World.getGremio(Coletor.getIDGremioPorMapaID(this._pelea._mapaReal.getID()));
                ...
                case 10: {
                    Personagens.Stats totalStats2 = this.getTotalStatsConBuff();
         ...
         */
        public void aplicarBuffInicioTurno(Fight pelea) {
            ArrayList<SpellEffect> arrayList = this._buffsPelea;
        ...
        public SpellEffect getBuff(int id) {
            for (SpellEffect EH : this._buffsPelea) {
        ...
        public void actualizarBuffsPelea() {
            ...
            block4: for (SpellEffect buff : this._buffsPelea) {
        ...
        public void addBuff(int efectoID, int valor, int turnos, int duracion, boolean desbufeable, int hechizoID, String args, Luchador lanzador, boolean veneno) {
            ...
            this._buffsPelea.add(new SpellEffect(efectoID, valor, this._puedeJugar ? turnos + 1 : turnos, duracion, desbufeable, lanzador, args, hechizoID, veneno));
            switch (efectoID) {
                case 106: {
        ...
        public int getIniciativa() {
            ...
            if (this._tipo == 5) {
                return World.getGremio(this._recaudador.getGremioID()).getNivel();
            }
            if (this._tipo == 7) {
                return 0;
            }
        ...
        public String xpStringLuch(String str) {
            if (this._perso != null) {
                ...
                return String.valueOf(World.getExpNivel((int)this._perso.getNivel())._personaje) + str + this._perso.getExperiencia() + str + World.getExpNivel((int)max)._personaje;
            }
        ...
        public MOB_tmpl.MobGrado getMob() {
            if (this._tipo == 2) {
                return this._mob;
            }
            return null;
        ...
        public int getTempPA(Fight pelea) {
            return pelea._tempLuchadorPA;
        }

        public int getTempPM(Fight pelea) {
            return pelea._tempLuchadorPM;
        }

        public void addTempPM(Fight pelea, int pm) {
            Fight fight = pelea;
            fight._tempLuchadorPM = (short)(fight._tempLuchadorPM + pm);
        }

        public void addTempPA(Fight pelea, int pa) {
            Fight fight = pelea;
        ...
        public synchronized void desbuffear() {
            ArrayList<SpellEffect> nuevosBuffs = new ArrayList<SpellEffect>();
            for (SpellEffect EH : this._buffsPelea) {
                if (!EH.esDesbufeable()) {
                    nuevosBuffs.add(EH);
                }
    ...
    public static class Trampa {
        ...
        private Spell.StatsHechizos _trampaHechizo;
        private Fight _pelea;
        private int _color;
        private boolean _visible = true;
        private int _paramEquipoDue\u00f1o = -1;

        public Trampa(Fight pelea, Luchador lanzador, Maps.Celda celda, byte tama\u00f1o, Spell.StatsHechizos trampaHechizo, int hechizo) {
            this._pelea = pelea;
            this._lanzador = lanzador;
            this._celda = celda;
        ...
        public void activaTrampa(Luchador trampeado) {
            ...
            ArrayList<Maps.Celda> celdas = new ArrayList<Maps.Celda>();
            ...
            for (int a = 0; a < this._tama\u00f1o; ++a) {
                ...
                celdas2.addAll(celdas);
                for (Maps.Celda aCelda : celdas2) {
                    char[] arrc = dirs;
                    int n = dirs.length;
                    for (int i = 0; i < n; ++i) {
                        char d = arrc[i];
                        Maps.Celda celda = this._pelea.getMapaCopia().getCelda(Pathfinding.getSigIDCeldaMismaDir(aCelda.getID(), d, this._pelea.getMapaCopia(), true));
