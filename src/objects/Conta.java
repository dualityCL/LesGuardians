...
public class Conta {
    ...
    private Personagens _tempPerso;
    ...
    private ArrayList<Dragossauros> _establo = new ArrayList();
    private boolean _muteado = false;
    public long _tiempoMuteado;
    public int _posicion = -1;
    private int _primeraVez;
    private Map<Integer, ArrayList<Mercador.ObjetoMercadillo>> _objMercadillos;
    private Map<Integer, Personagens> _personajes = new TreeMap<Integer, Personagens>();
    private int _regalo;
    public long _horaMuteada = 0L;

    public Conta(int ID, String nombre, String password, String apodo, String pregunta, String respuesta, int nivelGM, int vip, boolean baneado, String ultimaIP, String ultimaConeccion, String banco, long kamasBanco, String amigos, String enemigos, String establo, int primeravez, int regalo) {
        this._guid = ID;
        ...
        this._objMercadillos = World.getMisObjetos(this._guid);
        ...
        for (String d : establo.split(";")) {
            try {
                Dragossauros DP = World.getDragopavoPorID(Integer.parseInt(d));
                if (DP == null) continue;
    ...
    public ArrayList<Dragossauros> getEstablo() {
    ...
    public Personagens getTempPersonaje() {
    ...
    public static boolean cuentaLogin(String nombre, String contrase\u00f1a, String codigoLlave) {
        Conta cuenta = World.getCuentaPorNombre(nombre);
    ...
    public Map<Integer, Personagens> getPersonajes() {
    ...
    public boolean crearPj(String nombre, int sexo, int clase, int color1, int color2, int color3) {
        Personagens perso = Personagens.crearPersonaje(nombre, sexo, clase, color1, color2, color3, this);
        if (perso == null) {
            return false;
        }
        this._personajes.put(perso.getID(), perso);
    ...
    public void borrarPerso(int id) {
        ...
        World.eliminarPj(this._personajes.get(id), true);
    ...
    public void setTempPerso(Personagens perso) {
    ...
    public synchronized void resetTodosPjs() {
        try {
          for (Personagens perso : this._personajes.values()) {
            if (!perso.enLinea())
              continue; 
            if (perso.getIntercambio() != null)
              perso.getIntercambio().cancel(); 
            Fight pelea = perso.getPelea();
    ...
    public void mensajeAAmigos() {
        for (int i : this._idsAmigos) {
            Personagens perso = World.getPersonaje(i);
    ...
    public String stringListaAmigos() {
        ...
        for (int i : this._idsAmigos) {
            Personagens perso;
            Conta cuenta = World.getCuenta(i);
    ...
    public void addAmigo(int id) {
        ...
        if (!this._idsAmigos.contains(id)) {
            ...
            Conta amigo = World.getCuenta(id);
    ...
    public synchronized String stringIDsEstablo() {
        ...
        for (Dragossauros DP : this._establo) {
            if (primero) {
                str = str.append(";");
            }
    ...
    public boolean recuperarObjeto(int lineaID, int cantidad) {
        ...
        Mercador.ObjetoMercadillo objMerca = null;
        try {
            for (Mercador.ObjetoMercadillo tempEntry : this._objMercadillos.get(idPuestoMerca)) {
        ...
        World.getPuestoMerca(idPuestoMerca).borrarObjMercaDelPuesto(objMerca);
        return true;
    }

    public Mercador.ObjetoMercadillo[] getObjMercaDePuesto(int idPuestoMerca) {
        if (this._objMercadillos.get(idPuestoMerca) == null) {
            return new Mercador.ObjetoMercadillo[1];
        }
        Mercador.ObjetoMercadillo[] listaObjMercadillos = new Mercador.ObjetoMercadillo[20];
