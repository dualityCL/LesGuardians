...
public class Personaje {
    ...
    public void addHechizosSetClase(int hechizo, int efecto, int modificacion) {
        if (!this._hechizosSetClase.containsKey(hechizo)) {
            this._hechizosSetClase.put(hechizo, new Mundo.Duo<Integer, Integer>(efecto, modificacion));
        }
    }
    ...
    public Personaje(int id, String nombre, int sexo, int clase, int color1, int color2, int color3, long kamas, int pts, int capital, int energia, int nivel, long exp, int talla, int gfxID, byte alineacion, int cuenta, Map<Integer, Integer> stats, int mostrarAmigos, byte mostarAlineacion, String canal, short mapa, int celda, String inventario, int pdvPorc, String hechizos, String ptoSalvada, String oficios, int xpMontura, int montura, int honor, int deshonor, int nivelAlineacion, String zaaps, byte titulo, int esposoId, String tienda, int mercante, int ScrollFuerza, int ScrollInteligencia, int ScrollAgilidad, int ScrollSuerte, int ScrollVitalidad, int ScrollSabiduria, int restriccionesA, int restriccionesB, int encarnacion) {
        ...
        if (montura != -1) {
            this._montura = Mundo.getDragopavoPorID(montura);
        }
        this._talla = talla;
        this._gfxID = gfxID;
        this._xpDonadaMontura = xpMontura;
        this._baseStats = new Stats(stats, true, this);
        this._cuenta = Mundo.getCuenta(cuenta);
        try {
        ...
        this._canales = canal;
        this._mapa = Mundo.getMapa(mapa);
        this._puntoSalvado = ptoSalvada;
        if (this._mapa == null) {
            this._mapa = Mundo.getMapa((short)this.mapaClase());
        ...
        for (String item : inventario.split("\\|")) {
            int idObj;
            if (item.equals("") || (obj = Mundo.getObjeto(idObj = Integer.parseInt((infos = item.split(":"))[0]))) == null) continue;
        ...
        for (String item : tienda.split("\\|")) {
            int idObjeto;
            if (item.equals("") || (obj = Mundo.getObjeto(idObjeto = Integer.parseInt((infos = item.split(":"))[0]))) == null) continue;
        ...
        this._recuperarVida = new Timer(1000, new ActionListener(){
            ...
            public void actionPerformed(ActionEvent e) {
                Personaje.this.regenerarPuntoAPunto();
            }
        });
        ...
        if (!oficios.equals("")) {
            for (String aJobData : oficios.split(";")) {
                ...
                try {
                    ...
                    if (pos == -1) continue;
                    Oficio.StatsOficio statsOficio = this._statsOficios.get(pos);
        ...
        if (mascota != null) {
            this._mascota = Mundo.getMascota(mascota.getID());
        }
    }

    public Personaje(int id, String nombre, int sexo, int clase, int color1, int color2, int color3, int nivel, int talla, int gfxid, Map<Integer, Integer> stats, Map<Integer, Objeto> objetos, int pdvPorc, byte mostarAlineacion, int montura, int nivelAlineacion, byte alineacion) {
        this._ID = id;
        this._nombre = nombre;
        this._sexo = sexo;
        this._clase = clase;
    ...
    public static synchronized Personaje crearPersonaje(String nombre, int sexo, int clase, int color1, int color2, int color3, Cuenta cuenta) {
        String zaaps = "";
        ...
        Personaje nuevoPersonaje = new Personaje(Mundo.getSigIDPersonaje(), nombre, sexo, clase, color1, color2, color3, kamas, (nivel - 1) * 1, (nivel - 1) * 5, (short) 10000,  nivel, Mundo.getExpMinPersonaje(nivel), (short) 100,  Integer.parseInt(clase + "" + sexo), (byte) 0, cuenta.getID(), new TreeMap<Integer, Integer>(), 1, (byte) 0, "*#%!pi$:?", Constantes.getMapaInicio(clase), 314, objetos, 100, "", "7411,311", "", 0, -1, 0, 0, 0, zaaps, (byte) 0, 0, "", 0, 0, 0, 0, 0, 0, 0, 8192, 0, -1);
    ...
    public boolean boostearHechizo(int hechizoID) {
        ...
        if (this._puntosHechizo >= antNivel && Mundo.getHechizo(hechizoID).getStatsPorNivel(antNivel + 1).getReqNivel() <= this._nivel) {
    ...
    public void reiniciarCero() {
        ...
        this._energia = 10000;
        this._experiencia = Mundo.getExpMinPersonaje(this._nivel);
        this._montura = null;
        this._talla = 100;
        ...
        this._canales = "*#%!pi$:?";
        this._mapa = Mundo.getMapa(Constantes.getMapaInicio(this._clase));
    ...
    public String xpString(String c) {
        return String.valueOf(this._experiencia) + c + Mundo.getExpMinPersonaje(this._nivel) + c + Mundo.getExpMaxPersonaje(this._nivel);
    }
    ...
    public int getMaxPod() {
        ...
        for (Oficio.StatsOficio SO : this._statsOficios.values()) {
            pods += SO.getNivel() * 5;
    ...
    public void subirNivel(boolean addXp) {
        ...
        if (addXp) {
            this._experiencia = Mundo.getExpNivel((int)this._nivel)._personaje;
        }
    ...
    public void addExp(long xp) {
        ...
        while (this._experiencia >= Mundo.getExpMaxPersonaje(this._nivel) && this._nivel < LesGuardians.MAX_NIVEL) {
    ...
    public int aprenderOficio(Oficio oficio) {
        for (Map.Entry<Integer, Oficio.StatsOficio> entry : this._statsOficios.entrySet()) {
    ...
    public boolean teleportSinTodos(short nuevoMapaID, short nuevaCeldaID) {
        ...
        if ((nuevoMapa = Mundo.getMapa(nuevoMapaID)) == null || nuevoMapa.getCelda(nuevaCeldaID) == null || this._celda == null) {
            return false;
        }
        if (out != null) {
            SocketManager.ENVIAR_GA2_CARGANDO_MAPA(out, this._ID);
    ...
    private String analizarListaDrago() { //FIXME revisar buen funcionamiento
        ...
        if (this._cuenta.getEstablo().size() > 0) {
            for (Dragopavo DP : this._cuenta.getEstablo()) {
                if (primero) {
                    packet.append(";");
                }
    ...
    public void iniciarAccionOficio(int idTrabajo, Mapa.ObjetoInteractivo objInterac, GameThread.AccionDeJuego GA, Mapa.Celda celda) {
        Oficio.StatsOficio SO = this.getOficioPorTrabajo(idTrabajo);
        if (SO == null) {
            return;
        }
    ...
    public String stringListaZaap() {
        ...
        for (short i : this._zaaps) {
            if (Mundo.getMapa(i) == null || Mundo.getMapa(i).getSubArea().getArea().getSuperArea().getID() != subAreaID) continue;
            int costo = Fórmulas.calcularCosteZaap(this._mapa, Mundo.getMapa(i));
    ...
    public String stringListaPrismas() {
        ...
        for (Prisma prisma : Mundo.todosPrismas()) {
            short mapaID;
            if (prisma.getAlineacion() != this._alineacion || Mundo.getMapa(mapaID = prisma.getMapaID()) == null || Mundo.getMapa(mapaID).getSubArea().getArea().getSuperArea().getID() != subAreaID) continue;
            if (prisma.getEstadoPelea() == 0 || prisma.getEstadoPelea() == -2) {
                str.append("|" + mapaID + ";*");
                continue;
            }
            int costo = Fórmulas.calcularCosteZaap(this._mapa, Mundo.getMapa(mapaID));
    ...
    public static Personaje personajeClonado(Personaje perso, int id) {
        ...
        Personaje clon = new Personaje(id, perso._nombre, perso._sexo, perso._clase, perso._color1, perso._color2, perso._color3, perso._nivel, 100, perso._gfxID, stats, perso._objetos, 100, mostrarAlas, monturaID, nivelAlineacion, perso._alineacion);
        clon.esDoble(true);
        if (perso._montando) {
            clon._montando = true;
        }
    ...
    public String getEsposoListaAmigos() {
        Personaje esposo = Mundo.getPersonaje(this._esposo);
        StringBuffer str = new StringBuffer();
        if (esposo != null) {
    ...
    public void divorciar() {
        if (this._enLinea) {
            SocketManager.ENVIAR_Im_INFORMACION(this, "047;" + Mundo.getPersonaje(this._esposo).getNombre());
        }
        this._esposo = 0;
    }
    ...
    public String listaTienda() {
        ...
        for (Objeto objeto : this._tienda) {
            ...
            if ((tienda = Mundo.getObjTienda(idobjeto = objeto.getID())) == null) continue;
    ...
    public String analizarPrismas() {
        String str = "";
        Prisma prisma = Mundo.getPrisma(this._mapa.getSubArea().getPrismaID());
    ...
    public void restarVidaMascota(Mascota mascota) {
        Objeto masc = null;
        masc = mascota == null ? this.getObjPosicion(8) : Mundo.getObjeto(mascota.getID());
        if (masc != null) {
    ...
    public long expKoli() {
        switch (this._categoria) {
            case 1: {
                return (Mundo.getExpNivel((int)(this._nivel + 1))._personaje - Mundo.getExpNivel((int)this._nivel)._personaje) * 2L;
            }
            case 2: {
                return Mundo.getExpNivel((int)(this._nivel + 1))._personaje - Mundo.getExpNivel((int)this._nivel)._personaje;
            }
            case 3: {
                if (this._nivel == 300) {
                    return 0L;
                }
                return (Mundo.getExpNivel((int)(this._nivel + 1))._personaje - Mundo.getExpNivel((int)this._nivel)._personaje) / 6L;
            }
        }
        return 0L;
    }
    ...
    public static class Grupo {
        ...
        public ArrayList<Integer> getIDsPersos() {
            ...
            for (Personaje perso : this._personajesGrupo) {
        ...
        public int getNivelGrupo() {
            int nivel = 0;
            for (Personaje p : this._personajesGrupo) {
                nivel += p.getNivel();
            }
            return nivel;
        }
