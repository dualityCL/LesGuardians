...
public class Mapa {
	...
	private Map<Short, Combate> _peleas = new TreeMap<Short, Combate>();
	private ArrayList<MobModelo.MobGrado> _mobPosibles = new ArrayList();
	private Map<Integer, MobModelo.GrupoMobs> _grupoMobs = new TreeMap<Integer, MobModelo.GrupoMobs>();
	private Map<Integer, MobModelo.GrupoMobs> _grupoMobsFix = new TreeMap<Integer, MobModelo.GrupoMobs>();
	private Map<Integer, NPCModelo.NPC> _npcs = new TreeMap<Integer, NPCModelo.NPC>();
	private ArrayList<Integer> _mercante = new ArrayList();
	private ArrayList<Celda> _casillasObjInterac = new ArrayList();
	private byte _X = 0;
	private byte _Y = 0;
	private Mundo.SubArea _subArea;
	private Cercado _cercado;
	private byte _maxGrupoDeMobs = (byte) 3;
	private Map<Integer, ArrayList<Acción>> _accionFinPelea = new TreeMap<Integer, ArrayList<Acción>>();
	private byte _maxMobsPorGrupo;
	private byte _capacidadMercantes = 0;
	private String _mapData = "";
	private int _descripcion = 0;
	private MobModelo.GrupoMobs _ultimoGrupo = null;
	...
	public Mapa(short ID, String fecha, byte ancho, byte alto, String key, String posicionesDePelea, String mapData,
		...
		for (String mob : mobs.split("\\|")) {
			...
			if (id == 0 || nivel == 0 || Mundo.getMobModelo(id) == null
					|| Mundo.getMobModelo(id).getGradoPorNivel(nivel) == null)
				continue;
			_mobPosibles.add(Mundo.getMobModelo(id).getGradoPorNivel(nivel));
		}
	...
	public void aplicarAccionFinCombate(int tipo, ArrayList<Combate.Luchador> ganadores, boolean evento) {
		if (_accionFinPelea.get(tipo) == null) {
			return;
		}
		for (Acción accion : _accionFinPelea.get(tipo)) {
			if (accion.getCondiciones().equalsIgnoreCase("EVENTO") && !evento)
				continue;
			for (Combate.Luchador ganador : ganadores) {
				Personaje perso = ganador.getPersonaje();
				if (perso == null)
					continue;
	...
	public void delAccionFinPelea(int tipo, int aTipo) {
		if (_accionFinPelea.get(tipo) == null)
			return;
		ArrayList<Acción> copy = new ArrayList<Acción>();
	...
	public Map<Integer, NPCModelo.NPC> getNPCs() {
		return _npcs;
	}

	public NPCModelo.NPC addNPC(int npcID, short celdaID, byte dir) {
		NPCModelo npcModelo = Mundo.getNPCModelo(npcID);
		if (npcModelo == null || getCelda(celdaID) == null) {
			return null;
		}
		NPCModelo.NPC npc = new NPCModelo.NPC(npcModelo, _sigIDMapaInfo, celdaID, dir, npcModelo.getNombre());
	...
	public NPCModelo.NPC getNPC(int id) {
		return _npcs.get(id);
	}

	public NPCModelo.NPC removeNPC(int id) {
	...
	public String getGMsLuchadores() {
		String packets = "";
		for (Celda celda : _celdas.values()) {
			for (Combate.Luchador luchador : celda.getLuchadores().values()) {
	...
	public String getGMsGrupoMobs() {
		...
		String packets = "GM|";
		boolean primero = true;
		for (MobModelo.GrupoMobs grupoMob : _grupoMobs.values()) {
	...
	public String getGMsNPCs() {
		...
		String packets = "GM|";
		boolean primero = true;
		for (NPCModelo.NPC npc : _npcs.values()) {
	...
	public String getGMsMercantes() {
		...
		for (Integer idperso : _mercante) {
			try {
				String GM = Mundo.getPersonaje(idperso).stringGMmercante();
				if (GM.equals(""))
					continue;
				if (!primero) {
					packets = String.valueOf(packets) + "|+";
	...
	public String getGMsMonturas() {
		...
		for (Integer idmontura : _cercado.getListaCriando()) {
			String GM = Mundo.getDragopavoPorID(idmontura).getCriarMontura(_cercado);
			if (GM.equals(""))
				continue;
			if (!primero) {
				packets = String.valueOf(packets) + "|+";
	...
	public Map<Short, Combate> getPeleas() {
		return _peleas;
	}
	...
	public short getRandomCeldaIDLibre() {
		...
		for (Map.Entry<Short, Celda> entry : _celdas.entrySet()) {
			...
			if (!celda.esCaminable(true))
				continue;
			boolean ok = true;
			for (Map.Entry<Integer, MobModelo.GrupoMobs> entry2 : _grupoMobs.entrySet()) {
			...
			for (Entry<Integer, NPC> entry3 : _npcs.entrySet()) {
				if (((NPCModelo.NPC) entry3.getValue()).getCeldaID() != celda.getID())
					continue;
				ok = false;
			}
	...
	public void jugadorLLegaACelda(Personaje perso, short celdaID) {
	...
	public void spawnGrupo(byte alineacion, int cantGrupos, boolean enviar, short celdaID, MobModelo.GrupoMobs grupoM) {
		...
		for (int a = 1; a <= cantGrupos; ++a) {
			MobModelo.GrupoMobs grupo = new MobModelo.GrupoMobs(_sigIDMapaInfo, alineacion, _mobPosibles, this, celdaID,
					_maxMobsPorGrupo);
	...
	public void addGrupoMobTimer(boolean timer, short celda, String strGrupoMob, String condicion) {
		MobModelo.GrupoMobs grupoMob = new MobModelo.GrupoMobs(_sigIDMapaInfo, celda, strGrupoMob);
	...
	public void setUltimoGrupoMob(MobModelo.GrupoMobs mob) {
		_ultimoGrupo = mob;
	}

	public void addGrupoMobPermanente(short celda, String strGrupoMob) {
		MobModelo.GrupoMobs grupoMob;
		if (_ultimoGrupo == null) {
			grupoMob = new MobModelo.GrupoMobs(_sigIDMapaInfo, celda, strGrupoMob);
			if (grupoMob.getMobs().isEmpty()) {
				return;
			}
			--_sigIDMapaInfo;
	...
	public Combate iniciarPeleaPjVSPj(Personaje init1, Personaje init2, byte tipo) {
		short id = 1;
		if (!_peleas.isEmpty()) {
			id = (short) ((Short) _peleas.keySet().toArray()[_peleas.size() - 1] + 1);
		}
		Combate pelea = new Combate(id, this, init1, init2, tipo);
		_peleas.put(id, pelea);
		SocketManager.ENVIAR_fC_CANTIDAD_DE_PELEAS(this);
		return pelea;
	}

	public void iniciarPeleaVSMobs(Personaje perso, MobModelo.GrupoMobs grupoMob) {
		short id = 1;
		if (!_peleas.isEmpty()) {
			id = (short) ((Short) _peleas.keySet().toArray()[_peleas.size() - 1] + 1);
		}
		...
		_peleas.put(id, new Combate(id, this, perso, grupoMob, (byte)4));
		SocketManager.ENVIAR_fC_CANTIDAD_DE_PELEAS(this);
	}

	public void iniciarPeleaVSDopeul(Personaje perso, MobModelo.GrupoMobs grupoMob) {
		short id = 1;
		if (!_peleas.isEmpty()) {
			id = (short) ((Short) _peleas.keySet().toArray()[_peleas.size() - 1] + 1);
		}
		_peleas.put(id, new Combate(id, this, perso, grupoMob, (byte)3));
		SocketManager.ENVIAR_fC_CANTIDAD_DE_PELEAS(this);
	}
	...
	public void iniciarPeleaVSRecaudador(Personaje perso, Recaudador recaudador) {
		...
		_peleas.put(id, new Combate(id, this, perso, recaudador));
		SocketManager.ENVIAR_fC_CANTIDAD_DE_PELEAS(this);
	}
	...
	public void iniciarPeleaVSPrisma(Personaje perso, Prisma prisma) {
		...
		_peleas.put(id, new Combate(id, this, perso, prisma));
		SocketManager.ENVIAR_fC_CANTIDAD_DE_PELEAS(this);
	}
	...
	public Map<Integer, MobModelo.GrupoMobs> getMobGroups() {
		return _grupoMobs;
	}
	...
	public Combate getPelea(short id) {
	...
	public static class Celda {
		...
		private Map<Integer, Combate.Luchador> _luchadores = new TreeMap<Integer, Combate.Luchador>();
		private boolean _caminable = true;
		private boolean _lineaDeVista = true;
		private short _mapaID;
		private ArrayList<Acción> _celdaAccion;
		private ObjetoInteractivo _objetoInterac;
		private Objeto _objetoTirado;

		public Celda(Mapa mapa, short id, boolean caminable, boolean lineaDeVista, int objID) {
			_mapaID = mapa.getID();
			_id = id;
		...
		public void addLuchador(Combate.Luchador luchador) {
			if (!_luchadores.containsKey(luchador.getID())) {
				_luchadores.put(luchador.getID(), luchador);
			}
		}

		public void removerLuchador(Combate.Luchador luchador) {
		...
		public boolean lineaDeVistaBloqueada() {
			...
			for (Map.Entry<Integer, Combate.Luchador> entry : _luchadores.entrySet()) {
		...
		public Map<Integer, Combate.Luchador> getLuchadores() {
			if (_luchadores.isEmpty()) {
				return new TreeMap<Integer, Combate.Luchador>();
			}
			return _luchadores;
		}

		public Combate.Luchador getPrimerLuchador() {
			if (_luchadores.isEmpty()) {
				return null;
			}
			Iterator<Map.Entry<Integer, Combate.Luchador>> iterator = _luchadores.entrySet().iterator();
			if (iterator.hasNext()) {
				Map.Entry<Integer, Combate.Luchador> entry = iterator.next();
		...
		public void iniciarAccion(Personaje perso, GameThread.AccionDeJuego GA) {
			int accionID = -1;
			int celdaID = -1;
			try {
		...
		public void finalizarAccion(Personaje perso, GameThread.AccionDeJuego GA) {
			int accionID = -1;
			try {
				accionID = Integer.parseInt(GA._args.split(";")[1]);
			} catch (Exception exception) {
			...
			case 152: {
				...
				if (accionID == 102) {
					...
					obj = Mundo.getObjModelo(311).crearObjDesdeModelo(cantidad, false);
				} else if (accionID == 152) {
					Random rand = new Random();
					int x = rand.nextInt(6);
					if (x == 5) {
						SocketManager.enviar(perso, "cS" + perso.getID() + "|11");
						obj = Mundo.getObjModelo(6659).crearObjDesdeModelo(1, false);
					} else {
	...
	public static class Cercado {
		...
		public Cercado(int propietario, Mapa mapa, short celdaId, int tama\u00f1o, int gremio, int precio,
			...
			_tama\u00f1o = tama\u00f1o;
			_gremio = Mundo.getGremio(gremio);
			_mapa = mapa;
			_celda = celdaId;
			_precio = precio;
		...
		public synchronized void startMoverDrago() {
			if (_criando.size() > 0) {
				...
				for (Integer montura : _criando) {
					Dragopavo dragopavo = Mundo.getDragopavoPorID(montura);
					if (dragopavo == null)
						continue;
	...
	public static class ObjetoInteractivo {
		...
		private Mundo.ObjInteractivoModelo _interacMod;
		...
		public ObjetoInteractivo(Mapa mapa, Celda celda, int id) {
			...
			_interacMod = Mundo.getObjInteractivoModelo(_id);
			if (_interacMod != null) {
