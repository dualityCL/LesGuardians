...
public class NPC_tmpl {
    ...
    public NPC_tmpl(int id, int bonusValue, int gfxid, int escalaX, int escalaY, int sexo, int color1, int color2, int color3, String accesorios, int clip, int artWork, int preguntaID, String objVender, String nombre, long kamas) {
        this._id = id;
        ...
        for (String obj : objVender.split("\\,")) {
            try {
                ...
                Objeto.ObjetoModelo objModelo = World.getObjModelo(idModelo);
    ...
    public static class NPC {
        private NPC_tmpl _modeloBD;
        private short _celdaID;
        private int _id;
        private byte _orientacion;
        private String _nombre;

        public NPC(NPC_tmpl npcModelo, int id, short celda, byte o, String nombre) {
    ...
    public static class PreguntaNPC {
        ...
        public String stringArgParaDialogo(Personagens perso) {
            try {
                if (!Condiciones.validaCondiciones(null, perso, this._condicion)) {
                    return World.getPreguntaNPC(this._falsaPregunta).stringArgParaDialogo(perso);
                }
        ...
        public String stringGremio(Personagens perso, Guild gremio) {
            String str = String.valueOf(this._id);
            if (!this._args.equals("")) {
        ...
        private String analizarArgumentos(String args, Personagens perso) {
            String arg = args;
            arg = arg.replace("[nombre]", perso.getStringVar("nombre"));
            arg = arg.replace("[costoBanco]", perso.getStringVar("costoBanco"));
            arg = arg.replace("[lider]", World.liderRanking);
            arg = arg.replace("[npcKamas]", String.valueOf(World.getNPCModelo(408).getKamas()));
            return arg;
        }

        private String analizarArgumentosGremio(String args, Guild gremio) {
            String arg = args;
    ...
    public static class RespuestaNPC {
        private int _id;
        private ArrayList<Acao> _acciones = new ArrayList();
        ...
        public void addAccion(Acao accion) {
            ArrayList<Acao> c = new ArrayList<Acao>();
        ...
        public void aplicar(Personagens perso) {
            for (Acao accion : this._acciones) {
