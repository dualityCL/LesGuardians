...
public class Profissao {
    ...
    public Profissao(int id, String herramientas, String recetas) {
        this._id = id;
    ...
    public static int pesoStatActual(Objeto obj, String statsModif) {
        for (Map.Entry<Integer, Integer> entry : obj.getStats().getStatsComoMap().entrySet()) {
            ...
            int multi = 1;
            int coef = 1;
            byte tieneStatBase = Profissao.tieneStatBaseObjeto(obj, Integer.toHexString(statID));
            if (tieneStatBase == 2) {
                coef = 3;
    ...
    public static int pesoTotalActualObj(String statsModelo, Objeto obj) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            int multi = 1;
            int coef = 1;
            byte tieneStatBase = Profissao.tieneStatBaseObjeto(obj, stats[0]);
            if (tieneStatBase == 2) {
                coef = 3;
            } else if (tieneStatBase == 0) {
                coef = 8;
    ...
    public static int pesoTotalBaseObj(int idObjModelo) {
        ...
        int suma = 0;
        String statsModelo = "";
        statsModelo = World.getObjModelo(idObjModelo).getStringStatsObj();
    ...
    public static class AccionTrabajo {
        ...
        private Personagens _artesano;
        private Personagens _cliente;
        ...
        public void finalizarAccionTrabajo(Personagens perso, GameThread.AccionDeJuego AJ, Maps.Celda celda) {
            Maps.ObjetoInteractivo OI = celda.getObjetoInterac();
            if (this._esReceta) {
                ...
                Objeto.ObjetoModelo OM = World.getObjModelo(idObjModelo);
                if (OM != null) {
                    if (cant > 0) {
        ...
        public void iniciarAccionTrabajo(Personagens perso, Maps.ObjetoInteractivo OI, GameThread.AccionDeJuego AJ, Maps.Celda celda) {
        ...
        public void iniciarReceta() {
            ...
            try {
                ...
                int idReceta = World.getIDRecetaPorIngredientes(SO.getOficio().listaRecetaPorTrabajo(this._idTrabajoMod), ingredPorModelo);
                ...
                if (!exito) {
                    ...
                    Objeto objCreado = World.getObjModelo(idReceta).crearObjDesdeModelo(1, false);
                    if (firmado) {
                    ...
                    if (igual == null) {
                        World.addObjeto(objCreado, true);
        ...
        private void recetaForjaMagia() {
            ...
            block79: for (int idIngrediente : this._ingredientes.keySet()) {
                Objeto ing = World.getObjeto(idIngrediente);
                ...
                switch (idModelo) {
                    ...
                    default: {
                        ...
                        World.eliminarObjeto(idIngrediente);
            ...
            if (nivelRunaElemento > 0 && nivelRunaStats == 0) {
                ...
                if (!statStringObj.isEmpty()) {
                    pesoTotalActual = Profissao.pesoTotalActualObj(statStringObj, objAMaguear);
                    pesoActualStat = Profissao.pesoStatActual(objAMaguear, statAMaguear);
                }
                if ((pesoTotalBase = Profissao.pesoTotalBaseObj(objModeloID)) < 0) {
                    pesoTotalBase = 0;
                }
                if (pesoActualStat < 0) {
                    pesoActualStat = 0;
                ...
                float coef = 1.0f;
                byte tieneStatBase = Profissao.tieneStatBaseObjeto(objAMaguear, statAMaguear);
                byte tieneStatActual = Profissao.tieneStatActualObjeto(objAMaguear, statAMaguear);
            ...
            if (!exito) {
                ...
                if (objRunaFirma != null) {
                    ...
                    if (nuevaCant <= 0) {
                        ...
                        World.eliminarObjeto(objRunaFirma.getID());
                ...
                if (objModificador != null) {
                    ...
                    if (nuevaCant <= 0) {
                        ...
                        World.eliminarObjeto(objModificador.getID());
                ...
                if (romper == 100) {
                    ...
                    World.addObjeto(objAMaguear, false);
                ...
                if (nivelRunaElemento > 0 && nivelRunaStats == 0) {
                    for (SpellEffect EH : objAMaguear.getEfectos()) {
                    ...
                    byte actualStat = Profissao.tieneStatActualObjeto(objAMaguear, statAMaguear);
                    if (actualStat == 2) {
        ...
        public boolean iniciarTrabajoPago(Personagens artesano, Personagens cliente, ArrayList<World.Duo<Integer, Integer>> objArtesano, ArrayList<World.Duo<Integer, Integer>> objCliente) {
            int jet;
            boolean exito;
            if (!this._esReceta) {
                return false;
            ...
            boolean firmado = false;
            for (World.Duo<Integer, Integer> duo : objArtesano) {
                this._ingredientes.put((Integer)duo._primero, (Integer)duo._segundo);
            }
            for (World.Duo<Integer, Integer> duo : objCliente) {
                this._ingredientes.put((Integer)duo._primero, (Integer)duo._segundo);
            }
            if (this._idTrabajoMod == 1 || this._idTrabajoMod == 113 || this._idTrabajoMod == 115 || this._idTrabajoMod == 116 || this._idTrabajoMod == 117 || this._idTrabajoMod == 118 || this._idTrabajoMod == 119 || this._idTrabajoMod == 120 || this._idTrabajoMod >= 163 && this._idTrabajoMod <= 169) {
            ...
            if (!exito) {
                ...
                Objeto nuevoObj = World.getObjModelo(idReceta).crearObjDesdeModelo(1, false);
                if (firmado) {
        ...
        private boolean trabajoPagoFM() {
            ...
            block77: for (int idIngrediente : this._ingredientes.keySet()) {
                ...
                switch (idModelo) {
                    ...
                    default: {
                        ...
                        Personagens modificado = this._artesano.tieneObjetoID(idIngrediente) ? this._artesano : this._cliente;
                        int nuevaCant = objAMaguear.getCantidad() - 1;
            ...
            if (esCambioElemento > 0 && esCambiadoStats == 0) {
                ...
                if ((pesoTotalBase = Profissao.pesoTotalBaseObj(idObjModMaguear)) < 0) {
                    pesoTotalBase = 0;
                }
                if (pesoActualStat < 0) {
                    pesoActualStat = 0;
            ...
            if (!exito) {
                int romper;
                if (objRunaFirma != null) {
                    Personagens modificado = this._artesano.tieneObjetoID(objRunaFirma.getID()) ? this._artesano : this._cliente;
                    int nuevaCant = objRunaFirma.getCantidad() - 1;
                    if (nuevaCant <= 0) {
                ...
                if (objModificador != null) {
                    Personagens modificado = this._artesano.tieneObjetoID(objModificador.getID()) ? this._artesano : this._cliente;
                    int nuevaCant = objModificador.getCantidad() - 1;
                    if (nuevaCant <= 0) {
                ...
                if ((romper = Fórmulas.getRandomValor(1, 100)) == 100) {
                    this._romperse = true;
                    Personagens modificado = this._artesano.tieneObjetoID(idObjMaguear) ? this._artesano : this._cliente;
                    World.eliminarObjeto(idObjMaguear);
        ...
        public void variasMagueadas(int tiempo, Personagens perso) {
        ...
        public void ponerIngredUltRecet() {
            ...
            for (Map.Entry<Integer, Integer> e : this._ingredientes.entrySet()) {
                Objeto objeto = World.getObjeto(e.getKey());
    ...
    public static class StatsOficio {
        ...
        public StatsOficio(int posicion, Profissao oficio, int nivel, long xp) {
        ...
        public void iniciarTrabajo(int idTrabajo, Personagens perso, Maps.ObjetoInteractivo OI, GameThread.AccionDeJuego AJ, Maps.Celda celda) {
            for (AccionTrabajo AT : this._trabajosPoderRealizar) {
        ...
        public void finalizarTrabajo(int idTrabajo, Personagens perso, GameThread.AccionDeJuego AJ, Maps.Celda celda) {
        ...
        public void addXP(Personagens perso, long xp) {
            if (this._nivel > LesGuardians.MAX_NIVEL_PROF - 1) {
                return;
            }
            int exNivel = this._nivel;
            this._exp += xp;
            while (this._exp >= (long)World.getExpNivel((int)(this._nivel + 1))._oficio && this._nivel < LesGuardians.MAX_NIVEL_PROF) {
        ...
        public String getXpString(String s) {
            String str = String.valueOf(World.getExpNivel((int)this._nivel)._oficio) + s;
            str = String.valueOf(str) + this._exp + s;
            str = String.valueOf(str) + World.getExpNivel((int)(this._nivel < LesGuardians.MAX_NIVEL_PROF ? this._nivel + 1 : this._nivel))._oficio;
            return str;
        }

        public Profissao getOficio() {
            return this._oficio;
        }
