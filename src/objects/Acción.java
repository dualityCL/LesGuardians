...
public class Acción {
	...
	public Acción(int id, String args, String cond) {
		_ID = id;
		_args = args;
		_cond = cond;
	}
	...
	public void aplicar(Personaje perso, Personaje objetivo, int objUsadoID, short celda) {
		PrintWriter out;
		if (perso == null || perso.getCuenta().getEntradaPersonaje() == null) {
			return;
		}
		if (!(_cond.equalsIgnoreCase("") || _cond.equalsIgnoreCase("-1") || Condiciones.validaCondiciones(null, perso, _cond))) {
		...
		Objeto objUsar = Mundo.getObjeto(objUsadoID);
		switch (_ID) {
		case -2: {
			try {
		...
		case 0: {
			try {
				...
				short nuevaCeldaID = Short.parseShort(_args.split(",", 2)[1]);
		...
		case 2: {
			try {
				...
				if (cantidad > 0) {
					Objeto.ObjetoModelo OM = Mundo.getObjModelo(ID);
					if (OM == null) {
						return;
					}
		...
		case 6: {
			try {
				...
				if (idOficio == 2 || idOficio == 11 || idOficio == 13 || idOficio == 14 || idOficio == 15
						|| idOficio == 16 || idOficio == 17 || idOficio == 18 || idOficio == 19 || idOficio == 20
						|| idOficio == 24 || idOficio == 25 || idOficio == 26 || idOficio == 27 || idOficio == 28
						|| idOficio == 31 || idOficio == 36 || idOficio == 41 || idOficio == 56 || idOficio == 58
						|| idOficio == 60 || idOficio == 65) {
					...
					perso.aprenderOficio(Mundo.getOficio(idOficio));
					return;
				}
				if (idOficio != 43 && idOficio != 44 && idOficio != 45 && idOficio != 46 && idOficio != 47
						&& idOficio != 48 && idOficio != 49 && idOficio != 50 && idOficio != 62 && idOficio != 63
						&& idOficio != 64)
					return;
				if (perso.getOficioPorID(17) != null && perso.getOficioPorID(17).getNivel() >= 65 && idOficio == 43
						|| perso.getOficioPorID(11) != null && perso.getOficioPorID(11).getNivel() >= 65
								&& idOficio == 44
						|| perso.getOficioPorID(14) != null && perso.getOficioPorID(14).getNivel() >= 65
								&& idOficio == 45
						|| perso.getOficioPorID(20) != null && perso.getOficioPorID(20).getNivel() >= 65
								&& idOficio == 46
						|| perso.getOficioPorID(31) != null && perso.getOficioPorID(31).getNivel() >= 65
								&& idOficio == 47
						|| perso.getOficioPorID(13) != null && perso.getOficioPorID(13).getNivel() >= 65
								&& idOficio == 48
						|| perso.getOficioPorID(19) != null && perso.getOficioPorID(19).getNivel() >= 65
								&& idOficio == 49
						|| perso.getOficioPorID(18) != null && perso.getOficioPorID(18).getNivel() >= 65
								&& idOficio == 50
						|| perso.getOficioPorID(15) != null && perso.getOficioPorID(15).getNivel() >= 65
								&& idOficio == 62
						|| perso.getOficioPorID(16) != null && perso.getOficioPorID(16).getNivel() >= 65
								&& idOficio == 63
						|| perso.getOficioPorID(27) != null && perso.getOficioPorID(27).getNivel() >= 65
								&& idOficio == 64) {
					if (perso.getOficioPorID(idOficio) != null) {
						SocketManager.ENVIAR_Im_INFORMACION(perso, "111");
						return;
			...
			try {
				...
				case 124: {
					if (perso.getScrollSabiduria() >= 500) {
						SocketManager.ENVIAR_Im1223_MENSAJE_IMBORRABLE(perso,
								"Esse \u00e9 o m\u00e1ximo que pode ser scrollado.");
						return;
					}
		...
		case 12: {
			try {
				...
				PiedraAlma piedraAlma = (PiedraAlma) objUsar;
		...
		case 15: {
			try {
				...
				short nuevaCeldaID = Short.parseShort(_args.split(",")[1]);
		...
		case 17: {
			try {
				int oficioID = Integer.parseInt(_args.split(",")[0]);
				int exp = Integer.parseInt(_args.split(",")[1]);
		...
		case 23: {
			...
			Oficio.StatsOficio statsOficio = perso.getOficioPorID(oficio);
			if (statsOficio == null) {
				return;
			}
		...
		case 27: {
			try {
				...
				for (String mobYNivel : _args.split("\\|")) {
					...
					if (Mundo.getMobModelo(mobID) == null
							|| Mundo.getMobModelo(mobID).getGradoPorNivel(mobNivel) == null) {
				...
				MobModelo.GrupoMobs grupo = new MobModelo.GrupoMobs(perso.getMapa()._sigIDMapaInfo,
		...
		case 33: {
			...
			if (!perso.addObjetoSimilar(nuevo = Mundo.getObjModelo(objBoost).crearObjPosDesdeModelo(1, 20, false), true,
					-1)) {
		...
		case 34: {
			...
			if (objetoPos != null) {
				String maxStats = Objeto.ObjetoModelo.generarStatsModeloDB(objetoPos.getModelo().getStringStatsObj(),
						true);
				objetoPos.clearTodo();
		...
		case 36:
			try {
				...
				if (tutorial == 30) {
					int aleatorio = Fórmulas.getRandomValor(1, 200);
					if (aleatorio == 100)
						tutorial = 31;
				}
				Tutorial tuto = Mundo.getTutorial(tutorial);
				if (tuto == null)
					return;
		...
		case 37: {
			try {
				...
				if (perso.tieneObjModeloNoEquip(objNecesario, 1)) {
					...
					Objeto nuevoObj = Mundo.getObjModelo(objNuevo).crearObjDesdeModelo(1, false);
		...
		case 38: {
			try {
				short nuevaCelda = Short.parseShort(_args);
		...
		case 39: {
			try {
				NPCModelo npcModelo = Mundo.getNPCModelo(408);
		...
		case 41: {
			try {
				...
				if (Mundo.getMobModelo(mobID) == null || Mundo.getMobModelo(mobID).getGradoPorNivel(mobNivel) == null) {
					System.out.println("MobGrupo invalido mobID:" + mobID + " mobNivel:" + mobNivel);
					return;
				}
				mobGrupo = String.valueOf(mobGrupo) + mobID + "," + mobNivel + "," + mobNivel + ";";
		...
		case 42: {
			try {
				int idSet = Integer.parseInt(_args);
				Mundo.ObjetoSet OS = Mundo.getObjetoSet(idSet);
				if (OS == null) {
					return;
				}
		...
		case 50: {
			...
			if (perso.getMisionPVP() == null) {
				Personaje.MisionPVP mision = new Personaje.MisionPVP(0L, null, 0);
				perso.setMisionPVP(mision);
			}
			if (perso.getMisionPVP().getTiempoPVP() == 0L
					|| System.currentTimeMillis() - perso.getMisionPVP().getTiempoPVP() > 600000L) {
				Personaje tempP = null;
				ArrayList<Personaje> victimas = new ArrayList<Personaje>();
				for (Personaje victima : Mundo.getPJsEnLinea()) {
					if (victima == null || victima == perso || victima.getAlineacion() == perso.getAlineacion()
							|| victima.getAlineacion() == 0 || victima.getAlineacion() == 3 || !victima.mostrarAlas()
							|| victima.getCuenta().getActualIP().compareTo(perso.getCuenta().getActualIP()) == 0
							|| victima.getNombre().equalsIgnoreCase(perso.getUltimaMision())
							|| perso.getNivel() + 20 < victima.getNivel() || perso.getNivel() - 20 > victima.getNivel())
						continue;
					victimas.add(victima);
				}
				if (victimas.size() == 0) {
					SocketManager.ENVIAR_cs_CHAT_MENSAJE(perso,
							"<b>Thomas Sacre</b> : Aucune cible pour l'instant , reviens plus tard.", "000000");
					return;
				}
				tempP = (Personaje) victimas.get(Fórmulas.getRandomValor(0, victimas.size() - 1));
				String nombreVict = tempP.getNombre();
				SocketManager.ENVIAR_cs_CHAT_MENSAJE(perso, "<b>Thomas Sacre</b> : Seu alvo \u00e9 " + nombreVict + ".",
						"000000");
				...
				Objeto.ObjetoModelo objModelo = Mundo.getObjModelo(10085);
				...
				SocketManager.ENVIAR_cs_CHAT_MENSAJE(perso,
						"<b>Thomas Sacre</b> : Voc\u00ea j\u00e1 est\u00e1 em busca de algu\u00e9m, volte ap\u00f3s matar seu alvo.",
						"000000");
			}
			return;
		}
		case 51: {
			try {
				...
				Personaje victima = Mundo.getPjPorNombre(nombreCazar);
				...
				if (victima.esFantasma()) {
					SocketManager.ENVIAR_Im1223_MENSAJE_IMBORRABLE(perso,
							"Seu alvo est\u00e1 em <b>Modo Fantasma</b>.");
					return;
				}
				int kamas = objUsar.getRecompensaKamas();
				if (perso.getMisionPVP() == null) {
					perso.setMisionPVP(new Personaje.MisionPVP(0L, victima, kamas));
				} else {
					Personaje.MisionPVP mision = perso.getMisionPVP();
		...
		case 52: {
			...
			String nombreCazar = "";
			nombreCazar = Mundo.getObjeto(objUsadoID).getNombreMision();
			if (nombreCazar == null) {
				return;
			}
			Personaje victima = Mundo.getPjPorNombre(nombreCazar);
			try {
				...
				if (perso.getMisionPVP() == null) {
					perso.setMisionPVP(new Personaje.MisionPVP(0L, victima, 0));
				} else {
					Personaje.MisionPVP mision = perso.getMisionPVP();
					mision.setPjMision(victima);
		...
		case 53: {
			try {
				...
				Mundo.Trueque trueque = new Mundo.Trueque(perso, objetopedir, objetodar);
		...
		case 55: {
			try {
				...
				long precio = Long.parseLong(_args.split(";")[0]);
				...
				if (perso.getKamas() >= precio) {
					perso.setKamas(perso.getKamas() - precio);
					new Acción(accion, args, "").aplicar(perso, null, -1, (short) -1);
					return;
				} else {
					SocketManager.ENVIAR_Im_INFORMACION(perso, "182");
				}
		...
		case 56: {
			try {
				...
				if (perso.tieneObjModeloNoEquip(objModelo, cant)) {
					...
					Objeto nuevoObj = Mundo.getObjModelo(objNuevo).crearObjDesdeModelo(cantNuevo, false);
					if (!perso.addObjetoSimilar(nuevoObj, true, -1)) {
						Mundo.addObjeto(nuevoObj, true);
						perso.addObjetoPut(nuevoObj);
						SocketManager.ENVIAR_OAKO_APARECER_OBJETO(out, nuevoObj);
		...
		case 59: {
			try {
				int idSet = Integer.parseInt(_args.split(";")[0]);
				int fichas = Integer.parseInt(_args.split(";")[1]);
				Mundo.ObjetoSet OS = Mundo.getObjetoSet(idSet);
				if (OS == null) {
					return;
				}
		...
		case 100: {
			...
			int habilidad = 0;
			habilidad = _args.split(",").length == 2 ? Fórmulas.getRandomValor(Integer.parseInt(_args.split(",")[0]),
					Integer.parseInt(_args.split(",")[1])) : Integer.parseInt(_args);
			Dragopavo montura = perso.getMontura();
			montura.setHabilidad(String.valueOf(habilidad));
			perso.setMontura(montura);
			SocketManager.ENVIAR_Re_DETALLES_MONTURA(perso, "+", Mundo.getDragopavoPorID(montura.getID()));
		...
		case 101: {
			if (perso.getSexo() == 0 && perso.getCelda().getID() == 282
				...
				Mundo.addEsposo(perso.getSexo(), perso);
				return;
			} else {
		...
		case 102: {
			Mundo.discursoSacerdote(perso, perso.getMapa(), perso.getConversandoCon());
			return;
		}
		case 103: {
		...
		case 200: {
			try {
				...
				if (cercado.getCantObjColocados() < cercado.getCantObjMax()) {
					...
					SocketManager.ENVIAR_GDO_PONER_OBJETO_CRIA(mapa,
							String.valueOf(celda) + ";" + idModelo + ";1;1000;1000");
					perso.borrarObjetoEliminar(objUsadoID, 1, true);
					return;
				} else {
					SocketManager.ENVIAR_Im1223_MENSAJE_IMBORRABLE(perso,
							"Esse padoque j\u00e1 est\u00e1 no <b>limite</b> de itens de cria\u00e7\u00e3o.");
				}
				return;
			} catch (Exception mapa) {
				// empty catch block
		...
		case 201: {
			try {
				...
				Mundo.Area area = subarea.getArea();
				...
				if (mapa.esArena() || mapa.esCasa() || mapa.esTaller() || mapa.getID() > 13000
						|| mapa.getAncho() != 15 && mapa.getAncho() != 19) {
				...
				Prisma prisma = new Prisma(Mundo.getSigIDPrisma(), (byte) alineacion, (byte) 1, (short) mapa.getID(),
						(short) celdapj, 0, (short) -1);
		...
		case 228: {
			...
			Animación animacion = Mundo.getAnimacion(animacionID);
			if (perso.getPelea() != null) {
				return;
			}
			...
			SocketManager.ENVIAR_GA_ACCION_JUEGO_AL_MAPA(perso.getMapa(), "0", 228,
					String.valueOf(perso.getID()) + ";" + celda + "," + Animación.preparaAGameAccion(animacion), "");
			return;
		}
		default:
			System.out.println("Accion ID = " + _ID + " no implantada");
		}
	}
