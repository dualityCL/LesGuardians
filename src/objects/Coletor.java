...
public class Coletor {
    ...
    public Coletor(int ID, short mapa, short celdaID, byte orientacion, int gremioID, String N1, String N2, String items, long kamas, long xp) {
        this._id = ID;
        this._mapaID = mapa;
        ...
        for (String item : items.split("\\|")) {
            ...
            int id;
            Objeto obj;
            if (item.equals("") || (obj = World.getObjeto(id = Integer.parseInt((infos = item.split(":"))[0]))) == null) continue;
    ...
    public static String enviarGMDeRecaudador(Maps mapa) {
        String packet = "GM|";
        boolean primero = true;
        Map<Integer, Coletor> todosRecaudadores = World.getTodosRecaudadores();
        for (Map.Entry<Integer, Coletor> recau : todosRecaudadores.entrySet()) {
            Guild G;
            Coletor recaudador = recau.getValue();
            if (recau.getValue()._estadoPelea > 0 || recaudador._mapaID != mapa.getID()) continue;
            if (!primero) {
                packet = String.valueOf(packet) + "|";
            }
            if ((G = World.getGremio(recaudador._gremioID)) == null) {
    ...
    public void borrarRecaudador(int idRecaudador) {
        for (Objeto obj : this._objetos.values()) {
            World.eliminarObjeto(obj._id);
        }
        World.borrarRecaudador(idRecaudador);
    }

    public void borrarRecauPorRecolecta(int idRecaudador, Personagens perso) {
    ...
    public static String analizarRecaudadores(int gremioID) {
        ...
        for (Map.Entry<Integer, Coletor> recau : todosRecaudadores.entrySet()) {
            Coletor recaudador = recau.getValue();
            if (recaudador.getGremioID() != gremioID) continue;
            Maps mapa = World.getMapa(recaudador.getMapaID());
            if (primero) {
                packet = String.valueOf(packet) + "|";
            }
            packet = String.valueOf(packet) + Integer.toString(recaudador.getID(), 36) + ";" + recaudador.getN1() + "," + recaudador.getN2() + ";";
            ...
            if (recaudador.getEstadoPelea() == 1) {
                packet = mapa.getPelea(recaudador.getPeleaID()) == null ? String.valueOf(packet) + "45000;" : String.valueOf(packet) + recaudador.getTiempoTurno() + ";";
    ...
    public static int getIDGremioPorMapaID(int id) {
        for (Map.Entry<Integer, Coletor> recau : World.getTodosRecaudadores().entrySet()) {
    ...
    public static void analizarAtaque(Personagens perso, int gremioID) {
        for (Map.Entry<Integer, Coletor> recau : World.getTodosRecaudadores().entrySet()) {
            Coletor recaudador = recau.getValue();
            if (recaudador._estadoPelea <= 0 || recaudador._gremioID != gremioID) continue;
            SocketManager.ENVIAR_gITp_INFO_ATACANTES_RECAUDADOR(perso, Coletor.atacantesAlGremio(recaudador._id, recaudador._mapaID, recaudador._peleaID));
        }
    }

    public static void analizarDefensa(Personagens perso, int gremioID) {
        for (Map.Entry<Integer, Coletor> recau : World.getTodosRecaudadores().entrySet()) {
            Coletor recaudador = recau.getValue();
            if (recaudador._estadoPelea <= 0 || recaudador._gremioID != gremioID) continue;
            SocketManager.ENVIAR_gITP_INFO_DEFENSORES_RECAUDADOR(perso, Coletor.defensoresDelGremio(recaudador._id, recaudador._mapaID, recaudador._peleaID));
        }
    }
    ...
    public void borrarDesdeRecaudador(Personagens perso, int idObjeto, int cantidad) {
        Objeto RecauObj = World.getObjeto(idObjeto);
        ...
        if (PersoObj == null) {
            if (nuevaCant <= 0) {
                ...
                String str = "O+" + RecauObj.getID() + "|" + RecauObj.getCantidad() + "|" + RecauObj.getModelo().getID() + "|" + RecauObj.convertirStatsAString();
                SocketManager.ENVIAR_EsK_MOVER_A_TIENDA_COFRE_BANCO(perso, str);
            }
        } else if (nuevaCant <= 0) {
            this.borrarObjeto(idObjeto);
            World.eliminarObjeto(RecauObj.getID());
    ...
    public Maps getMapa() {
        return World.getMapa(this._mapaID);
    }
    ...
    public void moverPerco() {
        ...
        ArrayList<Maps.Celda> celdas = Pathfinding.pathMasCortoEntreDosCeldas(mapa, this._celdaID, celdadestino, 0);
        String pathstr = "";
        ...
        catch (InterruptedException celda) {
            // empty catch block
        }
        String path = pathstr;
        if (path.equals("")) {
        ...
        short resultado = Pathfinding.numeroMovimientos(this.getMapa(), this._celdaID, pathRef, null);
        if (resultado == 0) {
            return;
        }
        ...
        path = pathRef.get();
        if (resultado == -1000) {
            path = String.valueOf(CryptManager.getValorHashPorNumero(this.getOrientacion())) + CryptManager.celdaIDACodigo(this.getCeldalID());
        }
        SocketManager.ENVIAR_GA_ACCION_JUEGO_AL_MAPA(this.getMapa(), "0", 1, String.valueOf(this._id), "a" + CryptManager.celdaIDACodigo(this._celdaID) + path);
        this._celdaID = celdadestino;
        this._orientacion = (byte)tempDireccion;
    }
}
